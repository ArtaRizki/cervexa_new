package okhttp3.internal.p032ws;
...
import okhttp3.internal.p032ws.WebSocketReader;
...
public final class RealWebSocket implements WebSocket, WebSocketReader.FrameCallback {
    ...
    public RealWebSocket(Request request, WebSocketListener webSocketListener, Random random) {
        ...
        this.key = ByteString.m2315of(bArr).base64();
    ...
    public void connect(OkHttpClient okHttpClient) {
        OkHttpClient okHttpClientBuild = okHttpClient.newBuilder().protocols(ONLY_HTTP1).build();
        final int iPingIntervalMillis = okHttpClientBuild.pingIntervalMillis();
        final Request requestBuild = this.originalRequest.newBuilder().header("Upgrade", "websocket").header("Connection", "Upgrade").header("Sec-WebSocket-Key", this.key).header("Sec-WebSocket-Version", Constants.VIA_REPORT_TYPE_JOININ_GROUP).build();
