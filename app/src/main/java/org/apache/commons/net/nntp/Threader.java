...
public class Threader {
    ...
    public Threadable thread(Iterable<? extends Threadable> iterable) {
        ...
        HashMap<String, ThreadContainer> map = new HashMap<>();
        for (Threadable threadable : iterable) {
            if (!threadable.isDummy()) {
                buildContainer(threadable, map);
            }
        }
        if (map.isEmpty()) {
            return null;
        }
        ThreadContainer threadContainerFindRootSet = findRootSet(map);
        map.clear();
        pruneEmptyContainers(threadContainerFindRootSet);
        threadContainerFindRootSet.reverseChildren();
        gatherSubjects(threadContainerFindRootSet);
        if (threadContainerFindRootSet.next != null) {
            throw new RuntimeException("root node has a next:" + threadContainerFindRootSet);
        }
        for (ThreadContainer threadContainer = threadContainerFindRootSet.child; threadContainer != null; threadContainer = threadContainer.next) {
            if (threadContainer.threadable == null) {
                threadContainer.threadable = threadContainer.child.threadable.makeDummy();
            }
        }
        Threadable threadable2 = threadContainerFindRootSet.child != null ? threadContainerFindRootSet.child.threadable : null;
        threadContainerFindRootSet.flush();
        return threadable2;
    }

    private void buildContainer(Threadable threadable, HashMap<String, ThreadContainer> map) {
        String strMessageThreadId = threadable.messageThreadId();
        ThreadContainer threadContainer = map.get(strMessageThreadId);
        if (threadContainer != null) {
            if (threadContainer.threadable != null) {
                strMessageThreadId = "<Bogus-id:1>";
        ...
        if (threadContainer == null) {
            ...
            threadContainer.threadable = threadable;
            map.put(strMessageThreadId, threadContainer);
        }
        ...
        while (i < length) {
            String str = strArrMessageThreadReferences[i];
            ThreadContainer threadContainer3 = map.get(str);
            if (threadContainer3 == null) {
                threadContainer3 = new ThreadContainer();
                map.put(str, threadContainer3);
            }
    ...
    private ThreadContainer findRootSet(HashMap<String, ThreadContainer> map) {
        ThreadContainer threadContainer = new ThreadContainer();
        Iterator<Map.Entry<String, ThreadContainer>> it = map.entrySet().iterator();
        while (it.hasNext()) {
    ...
    private void gatherSubjects(ThreadContainer threadContainer) {
        ...
        for (ThreadContainer threadContainer5 = threadContainer.child; threadContainer5 != null; threadContainer5 = threadContainer5.next) {
            ...
            if (strSimplifiedSubject != null && strSimplifiedSubject.length() != 0 && ((threadContainer3 = (ThreadContainer) map.get(strSimplifiedSubject)) == null || ((threadContainer5.threadable == null && threadContainer3.threadable != null) || (threadContainer3.threadable != null && threadContainer3.threadable.subjectIsReply() && threadContainer5.threadable != null && !threadContainer5.threadable.subjectIsReply())))) {
                map.put(strSimplifiedSubject, threadContainer5);
                i++;
            }
        }
        if (i == 0) {
        ...
        while (threadContainer6 != null) {
            ...
            if (strSimplifiedSubject2 == null || strSimplifiedSubject2.length() == 0 || (threadContainer2 = (ThreadContainer) map.get(strSimplifiedSubject2)) == threadContainer6) {
