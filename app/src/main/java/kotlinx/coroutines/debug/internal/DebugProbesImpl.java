...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        if (objNewInstance != null) {
            objM2353constructorimpl = Result.m2353constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
            if (Result.m2359isFailureimpl(objM2353constructorimpl)) {
                objM2353constructorimpl = null;
            }
            return (Function1) objM2353constructorimpl;
        }
    ...
    private final void startWeakRefCleanerThread() {
        weakRefCleanerThread = ThreadsKt.thread$default(false, true, null, "Coroutines Debugger Cleaner", 0, new Function0<Unit>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl.startWeakRefCleanerThread.1
            ...
            public final void invoke2() {
                DebugProbesImpl.access$getCallerInfoCache$p(DebugProbesImpl.INSTANCE).runWeakRefQueueCleaningLoopUntilInterrupted();
            }
        }, 21, null);
    }
    ...
    private final <R> List<R> dumpCoroutinesInfoImpl(Function2<? super CoroutineOwner<?>, ? super CoroutineContext, ? extends R> create) {
        ReentrantReadWriteLock reentrantReadWriteLockAccess$getCoroutineStateLock$p = access$getCoroutineStateLock$p(this);
        ReentrantReadWriteLock.ReadLock lock = reentrantReadWriteLockAccess$getCoroutineStateLock$p.readLock();
        int i = 0;
        int readHoldCount = reentrantReadWriteLockAccess$getCoroutineStateLock$p.getWriteHoldCount() == 0 ? reentrantReadWriteLockAccess$getCoroutineStateLock$p.getReadHoldCount() : 0;
        for (int i2 = 0; i2 < readHoldCount; i2++) {
            lock.unlock();
        }
        ReentrantReadWriteLock.WriteLock writeLock = reentrantReadWriteLockAccess$getCoroutineStateLock$p.writeLock();
        writeLock.lock();
        try {
            if (!INSTANCE.isInstalled$kotlinx_coroutines_core()) {
                throw new IllegalStateException("Debug probes are not installed".toString());
            }
            List<CoroutineOwner> listSortedWith = CollectionsKt.sortedWith(INSTANCE.getCapturedCoroutines(), new Comparator<T>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$$special$$inlined$sortedBy$1
            ...
            ArrayList arrayList = new ArrayList();
            for (CoroutineOwner coroutineOwner : listSortedWith) {
                CoroutineContext context = coroutineOwner.info.getContext();
                R rInvoke = context != null ? create.invoke(coroutineOwner, context) : null;
                if (rInvoke != null) {
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        if (!(!Intrinsics.areEqual(state, DebugCoroutineInfoImplKt.RUNNING)) && thread != null) {
            try {
                Result.Companion companion = Result.INSTANCE;
                DebugProbesImpl debugProbesImpl = this;
                objM2353constructorimpl = Result.m2353constructorimpl(thread.getStackTrace());
            } catch (Throwable th) {
                Result.Companion companion2 = Result.INSTANCE;
                objM2353constructorimpl = Result.m2353constructorimpl(ResultKt.createFailure(th));
            }
            ...
            StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) objM2353constructorimpl;
            if (stackTraceElementArr != null) {
                ...
                Pair<Integer, Boolean> pairFindContinuationStartIndex = findContinuationStartIndex(i, stackTraceElementArr, coroutineTrace);
                int iIntValue = pairFindContinuationStartIndex.component1().intValue();
                boolean zBooleanValue = pairFindContinuationStartIndex.component2().booleanValue();
                if (iIntValue == -1) {
                    return coroutineTrace;
                }
                ArrayList arrayList = new ArrayList((((coroutineTrace.size() + i) - iIntValue) - 1) - (zBooleanValue ? 1 : 0));
                int i2 = i - (zBooleanValue ? 1 : 0);
    ...
    private final Pair<Integer, Boolean> findContinuationStartIndex(int indexOfResumeWith, StackTraceElement[] actualTrace, List<StackTraceElement> coroutineTrace) {
        int iFindIndexOfFrame = findIndexOfFrame(indexOfResumeWith - 1, actualTrace, coroutineTrace);
        if (iFindIndexOfFrame == -1) {
            return TuplesKt.m2297to(Integer.valueOf(findIndexOfFrame(indexOfResumeWith - 2, actualTrace, coroutineTrace)), true);
        }
        return TuplesKt.m2297to(Integer.valueOf(iFindIndexOfFrame), false);
    }
    ...
    public final List<DebugCoroutineInfo> dumpCoroutinesInfo() {
        ...
        try {
            ...
            List<CoroutineOwner> listSortedWith = CollectionsKt.sortedWith(INSTANCE.getCapturedCoroutines(), new Comparator<T>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$$special$$inlined$sortedBy$2
                /* JADX WARN: Multi-variable type inference failed */
                @Override // java.util.Comparator
                public final int compare(T t, T t2) {
                    return ComparisonsKt.compareValues(Long.valueOf(((DebugProbesImpl.CoroutineOwner) t).info.sequenceNumber), Long.valueOf(((DebugProbesImpl.CoroutineOwner) t2).info.sequenceNumber));
    ...
    public final List<DebuggerInfo> dumpDebuggerInfo() {
        ...
        try {
            ...
            List<CoroutineOwner> listSortedWith = CollectionsKt.sortedWith(INSTANCE.getCapturedCoroutines(), new Comparator<T>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$$special$$inlined$sortedBy$3
                /* JADX WARN: Multi-variable type inference failed */
                @Override // java.util.Comparator
                public final int compare(T t, T t2) {
                    return ComparisonsKt.compareValues(Long.valueOf(((DebugProbesImpl.CoroutineOwner) t).info.sequenceNumber), Long.valueOf(((DebugProbesImpl.CoroutineOwner) t2).info.sequenceNumber));
