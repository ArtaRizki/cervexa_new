...
public class GLDrawer2D implements IDrawer2dES2 {
    ...
    public GLDrawer2D(float[] fArr, float[] fArr2, boolean z) {
        ...
        int iMin = Math.min(fArr != null ? fArr.length : 0, fArr2 != null ? fArr2.length : 0) / 2;
        this.VERTEX_NUM = iMin;
        this.VERTEX_SZ = iMin * 2;
        this.mTexTarget = z ? ShaderConst.GL_TEXTURE_EXTERNAL_OES : ShaderConst.GL_TEXTURE_2D;
        FloatBuffer floatBufferAsFloatBuffer = ByteBuffer.allocateDirect(this.VERTEX_SZ * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
        this.pVertex = floatBufferAsFloatBuffer;
        floatBufferAsFloatBuffer.put(fArr);
        this.pVertex.flip();
        FloatBuffer floatBufferAsFloatBuffer2 = ByteBuffer.allocateDirect(this.VERTEX_SZ * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
