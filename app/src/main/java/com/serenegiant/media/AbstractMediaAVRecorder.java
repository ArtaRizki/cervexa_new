...
public abstract class AbstractMediaAVRecorder extends Recorder {
    ...
    public AbstractMediaAVRecorder(Context context, IRecorder.RecorderCallback recorderCallback, String str, String str2, int i) throws IOException {
        ...
        if (i > 0 && SAFUtils.hasStorageAccess(context, i)) {
            ...
            if (TextUtils.isEmpty(str)) {
                dateTimeString = FileUtils.getDateTimeString();
            } else {
                dateTimeString = str + FileUtils.getDateTimeString();
            }
        ...
        try {
            String string = FileUtils.getCaptureFile(context, Environment.DIRECTORY_MOVIES, str, str2, 0).toString();
    ...
    public AbstractMediaAVRecorder(Context context, IRecorder.RecorderCallback recorderCallback, int i, String str, String str2) throws Exception {
        ...
        if (i > 0 && SAFUtils.hasStorageAccess(context, i) && (storageFile = SAFUtils.getStorageFile(context, i, str, "*/*", str2)) != null) {
            ...
            ParcelFileDescriptor parcelFileDescriptorOpenFileDescriptor = context.getContentResolver().openFileDescriptor(storageFile.getUri(), "rw");
            try {
                if (parcelFileDescriptorOpenFileDescriptor != null) {
                    setupMuxer(parcelFileDescriptorOpenFileDescriptor.getFd());
                    return;
                }
