...
public class MediaAudioDecoder extends MediaDecoder {
    ...
    protected int handlePrepare(MediaExtractor mediaExtractor) {
        int iSelectTrack = selectTrack(mediaExtractor, "audio/");
        if (iSelectTrack >= 0) {
            MediaFormat trackFormat = mediaExtractor.getTrackFormat(iSelectTrack);
    ...
    @Override // com.serenegiant.media.MediaDecoder
    protected MediaCodec createCodec(MediaExtractor mediaExtractor, int i, MediaFormat mediaFormat) throws Exception {
