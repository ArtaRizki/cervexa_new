        ...
        if (BuildCheck.isAndroid5()) {
            ...
            if (allNetworks != null) {
                ...
                for (Network network : allNetworks) {
                    ...
                    if (networkInfo != null) {
                        type |= networkInfo.isConnectedOrConnecting() ? 1 << networkInfo.getType() : 0;
                        type2 |= networkInfo.isConnected() ? 1 << networkInfo.getType() : 0;
                    }
                }
            } else {
                type = 0;
            ...
            for (int i = 0; i < length; i += 2) {
                NetworkInfo networkInfo2 = connectivityManager.getNetworkInfo(NETWORKS[i]);
                if (networkInfo2 != null) {
                    type |= networkInfo2.isConnectedOrConnecting() ? NETWORKS[i + 1] : 0;
                    type2 |= networkInfo2.isConnected() ? NETWORKS[i + 1] : 0;
                }
            }
        }
        NetworkInfo activeNetworkInfo = connectivityManager.getActiveNetworkInfo();
        int type3 = activeNetworkInfo != null ? 1 << activeNetworkInfo.getType() : 0;
        synchronized (sSync) {
            sIsConnectedOrConnecting = type;
            sIsConnected = type2;
            sActiveNetworkMask = type3;
            sSync.notifyAll();
        }
        callOnNetworkChanged(type, type2, type3);
        Intent intent2 = new Intent(ACTION_LOCAL_CONNECTIVITY_CHANGE);
        intent2.putExtra(KEY_NETWORK_CHANGED_IS_CONNECTED_OR_CONNECTING, type);
        intent2.putExtra(KEY_NETWORK_CHANGED_IS_CONNECTED, type2);
        intent2.putExtra(KEY_NETWORK_CHANGED_ACTIVE_NETWORK_MASK, type3);
