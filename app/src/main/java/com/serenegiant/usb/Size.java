...
public class Size implements Parcelable {
    ...
    public void updateFrameRate() {
        ...
        if (i > 0) {
            ...
            try {
                int iMin = Math.min(this.intervals[0], this.intervals[1]);
                int iMax = Math.max(this.intervals[0], this.intervals[1]);
                int i3 = this.intervals[2];
                if (i3 > 0) {
                    int i4 = 0;
                    for (int i5 = iMin; i5 <= iMax; i5 += i3) {
                        i4++;
                    }
                    this.fps = new float[i4];
                    int i6 = 0;
                    while (iMin <= iMax) {
                        this.fps[i6] = 1.0E7f / iMin;
                        iMin += i3;
                        i6++;
                    }
                } else {
    ...
    public String toString() {
        ...
        return String.format(Locale.US, "Size(%dx%d@%4.1f,type:%d,frame:%d,index:%d,%s)", Integer.valueOf(this.width), Integer.valueOf(this.height), Float.valueOf(currentFrameRate), Integer.valueOf(this.type), Integer.valueOf(this.frame_type), Integer.valueOf(this.index), this.frameRates);
    }
}
