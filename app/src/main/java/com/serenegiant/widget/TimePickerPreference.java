...
public class TimePickerPreference extends DialogPreference {
    ...
    protected void onSetInitialValue(boolean z, Object obj) {
        long persistedLong = this.mDefaultValue;
        if (persistedLong <= 0) {
            persistedLong = System.currentTimeMillis();
        }
        if (z) {
            try {
                persistedLong = getPersistedLong(persistedLong);
            } catch (Exception unused) {
            }
            this.calendar.setTimeInMillis(persistedLong);
        } else if (obj != null) {
