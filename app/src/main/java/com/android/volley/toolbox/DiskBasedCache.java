...
public class DiskBasedCache implements Cache {
    ...
    public synchronized void clear() {
        ...
        this.mEntries.clear();
        this.mTotalSize = 0L;
        VolleyLog.m17d("Cache cleared.", new Object[0]);
    }
    ...
    */
    public synchronized com.android.volley.Cache.Entry get(java.lang.String r12) {
        ...
        L4a:
            ...
            com.android.volley.VolleyLog.m17d(r7, r5)     // Catch: java.lang.Throwable -> L8c
    ...
    public synchronized void initialize() {
        if (!this.mRootDirectory.exists()) {
            if (!this.mRootDirectory.mkdirs()) {
                VolleyLog.m18e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
            }
            return;
        }
        File[] fileArrListFiles = this.mRootDirectory.listFiles();
    ...
    public synchronized void put(String str, Cache.Entry entry) {
        ...
        try {
            ...
            if (!cacheHeader.writeHeader(bufferedOutputStream)) {
                ...
                VolleyLog.m17d("Failed to write header for %s", fileForKey.getAbsolutePath());
            ...
            VolleyLog.m17d("Could not clean up file %s", fileForKey.getAbsolutePath());
        }
    }
    ...
    public synchronized void remove(String str) {
        ...
        removeEntry(str);
        if (!zDelete) {
            VolleyLog.m17d("Could not delete cache entry for key=%s, filename=%s", str, getFilenameForKey(str));
        }
    }
    ...
    private void pruneIfNeeded(int i) {
        ...
        if (VolleyLog.DEBUG) {
            VolleyLog.m20v("Pruning old cache entries.", new Object[0]);
        }
        ...
        while (it.hasNext()) {
            ...
            if (getFileForKey(value.key).delete()) {
                ...
                j = j2;
                VolleyLog.m17d("Could not delete cache entry for key=%s, filename=%s", value.key, getFilenameForKey(value.key));
            }
            it.remove();
            i2++;
        ...
        if (VolleyLog.DEBUG) {
            VolleyLog.m20v("pruned %d files, %d bytes, %d ms", Integer.valueOf(i2), Long.valueOf(this.mTotalSize - j3), Long.valueOf(SystemClock.elapsedRealtime() - jElapsedRealtime));
        }
    }
