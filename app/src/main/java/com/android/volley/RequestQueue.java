    ...
    public <T> Request<T> add(Request<T> request) {
        ...
        synchronized (this.mWaitingRequests) {
            ...
            if (this.mWaitingRequests.containsKey(cacheKey)) {
                ...
                if (VolleyLog.DEBUG) {
                    VolleyLog.m20v("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
                }
            } else {
    ...
    <T> void finish(Request<T> request) {
        ...
        if (request.shouldCache()) {
            synchronized (this.mWaitingRequests) {
                ...
                if (queueRemove != null) {
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m20v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(queueRemove.size()), cacheKey);
                    }
