        ...
        public boolean onTouch(View view, MotionEvent motionEvent) {
            ...
            if (action != 0) {
                if (action == 2 && ZoomableSurfaceView.this.isSingleTouch) {
                    ZoomableSurfaceView.this.animate().x(motionEvent.getRawX() + this.f1939dX).y(motionEvent.getRawY() + this.f1940dY).setDuration(0L).start();
                    ZoomableSurfaceView zoomableSurfaceView = ZoomableSurfaceView.this;
                    zoomableSurfaceView.checkDimension(zoomableSurfaceView);
                }
            } else {
                this.f1939dX = ZoomableSurfaceView.this.getX() - motionEvent.getRawX();
                this.f1940dY = ZoomableSurfaceView.this.getY() - motionEvent.getRawY();
            }
            return true;
        }
    }
