...
public class HorizontalBarHighlighter extends BarHighlighter {
    ...
    public Highlight getHighlight(float f, float f2) {
        ...
        Highlight highlightForX = getHighlightForX((float) valsForTouch.f1969y, f2, f);
        if (highlightForX == null) {
            return null;
        }
        IBarDataSet iBarDataSet = (IBarDataSet) barData.getDataSetByIndex(highlightForX.getDataSetIndex());
        if (iBarDataSet.isStacked()) {
            return getStackedHighlight(highlightForX, iBarDataSet, (float) valsForTouch.f1969y, (float) valsForTouch.f1968x);
        }
    ...
    protected List<Highlight> buildHighlights(IDataSet iDataSet, int i, float f, DataSet.Rounding rounding) {
        ...
        for (Entry entry : entriesForXValue) {
            ...
            arrayList.add(new Highlight(entry.getX(), entry.getY(), (float) pixelForValues.f1968x, (float) pixelForValues.f1969y, i, iDataSet.getAxisDependency()));
        }
        return arrayList;
    }
