...
public class BarHighlighter extends ChartHighlighter<BarDataProvider> {
    ...
    public Highlight getHighlight(float f, float f2) {
        ...
        if (iBarDataSet.isStacked()) {
            return getStackedHighlight(highlight, iBarDataSet, (float) valsForTouch.f1968x, (float) valsForTouch.f1969y);
        }
    ...
    public Highlight getStackedHighlight(Highlight highlight, IBarDataSet iBarDataSet, float f, float f2) {
        ...
        MPPointD pixelForValues = ((BarDataProvider) this.mChart).getTransformer(iBarDataSet.getAxisDependency()).getPixelForValues(highlight.getX(), ranges[closestStackIndex].f1959to);
        Highlight highlight2 = new Highlight(barEntry.getX(), barEntry.getY(), (float) pixelForValues.f1968x, (float) pixelForValues.f1969y, highlight.getDataSetIndex(), closestStackIndex, highlight.getAxis());
