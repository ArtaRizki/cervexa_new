    ...
    */
    ...
    static /* synthetic */ class C10682 {
        ...
        static {
            ...
            try {
                f1946x9c9dbef[Legend.LegendOrientation.HORIZONTAL.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            int[] iArr2 = new int[Legend.LegendHorizontalAlignment.values().length];
            f1945x2787f53e = iArr2;
            try {
                iArr2[Legend.LegendHorizontalAlignment.LEFT.ordinal()] = 1;
            } catch (NoSuchFieldError unused3) {
            }
            try {
                f1945x2787f53e[Legend.LegendHorizontalAlignment.RIGHT.ordinal()] = 2;
            } catch (NoSuchFieldError unused4) {
            }
            try {
                f1945x2787f53e[Legend.LegendHorizontalAlignment.CENTER.ordinal()] = 3;
            } catch (NoSuchFieldError unused5) {
            }
            int[] iArr3 = new int[Legend.LegendVerticalAlignment.values().length];
            f1947xc926f1ec = iArr3;
            try {
                iArr3[Legend.LegendVerticalAlignment.TOP.ordinal()] = 1;
            } catch (NoSuchFieldError unused6) {
            }
            try {
                f1947xc926f1ec[Legend.LegendVerticalAlignment.BOTTOM.ordinal()] = 2;
    ...
    public float getAngleForPoint(float f, float f2) {
        ...
        double d = f - centerOffsets.f1970x;
        double d2 = f2 - centerOffsets.f1971y;
        float degrees = (float) Math.toDegrees(Math.acos(d2 / Math.sqrt((d * d) + (d2 * d2))));
        if (f > centerOffsets.f1970x) {
    ...
    public void getPosition(MPPointF mPPointF, float f, float f2, MPPointF mPPointF2) {
        double d = f;
        double d2 = f2;
        mPPointF2.f1970x = (float) (((double) mPPointF.f1970x) + (Math.cos(Math.toRadians(d2)) * d));
        mPPointF2.f1971y = (float) (((double) mPPointF.f1971y) + (d * Math.sin(Math.toRadians(d2))));
    }
    ...
    public float distanceToCenter(float f, float f2) {
        ...
        MPPointF centerOffsets = getCenterOffsets();
        if (f > centerOffsets.f1970x) {
            f3 = f - centerOffsets.f1970x;
        } else {
            f3 = centerOffsets.f1970x - f;
        }
        if (f2 > centerOffsets.f1971y) {
            f4 = f2 - centerOffsets.f1971y;
        } else {
            f4 = centerOffsets.f1971y - f2;
        }
