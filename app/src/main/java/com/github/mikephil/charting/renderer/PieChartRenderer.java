...
public class PieChartRenderer extends DataRenderer {
    ...
    protected float calculateMinimumRadiusForSpacedSlice(MPPointF mPPointF, float f, float f2, float f3, float f4, float f5, float f6) {
        ...
        float fCos = mPPointF.f1970x + (((float) Math.cos(d)) * f);
        float fSin = mPPointF.f1971y + (((float) Math.sin(d)) * f);
        double d2 = (f5 + (f6 / 2.0f)) * 0.017453292f;
        return (float) (((double) (f - ((float) ((Math.sqrt(Math.pow(fCos - f3, 2.0d) + Math.pow(fSin - f4, 2.0d)) / 2.0d) * Math.tan(((180.0d - ((double) f2)) / 2.0d) * 0.017453292519943295d))))) - Math.sqrt(Math.pow((mPPointF.f1970x + (((float) Math.cos(d2)) * f)) - ((fCos + f3) / 2.0f), 2.0d) + Math.pow((mPPointF.f1971y + (((float) Math.sin(d2)) * f)) - ((fSin + f4) / 2.0f), 2.0d)));
    }
    ...
    protected void drawDataSet(Canvas canvas, IPieDataSet iPieDataSet) {
        ...
        while (i7 < entryCount) {
            ...
            if (Math.abs(iPieDataSet2.getEntryForIndex(i7).getY()) <= Utils.FLOAT_EPSILON || pieChartRenderer.mChart.needsHighlight(i7)) {
                ...
                i2 = entryCount;
                fArr = drawAngles;
                float fCos = centerCircleBox.f1970x + (((float) Math.cos(d)) * radius);
                float fSin = centerCircleBox.f1971y + (((float) Math.sin(d)) * radius);
                if (f12 >= 360.0f && f12 % 360.0f <= Utils.FLOAT_EPSILON) {
                    f3 = phaseX;
                    pieChartRenderer.mPathBuffer.addCircle(centerCircleBox.f1970x, centerCircleBox.f1971y, radius, Path.Direction.CW);
                } else {
                    f3 = phaseX;
                ...
                float f13 = f12;
                pieChartRenderer.mInnerRectBuffer.set(centerCircleBox.f1970x - holeRadius, centerCircleBox.f1971y - holeRadius, centerCircleBox.f1970x + holeRadius, centerCircleBox.f1971y + holeRadius);
                if (!z) {
                    f4 = holeRadius;
                    f2 = rotationAngle;
                    f5 = f13;
                    ...
                    if (f12 >= 360.0f && f7 % 360.0f <= Utils.FLOAT_EPSILON) {
                        ...
                        pieChartRenderer.mPathBuffer.addCircle(mPPointF2.f1970x, mPPointF2.f1971y, holeRadius, Path.Direction.CCW);
                        f2 = rotationAngle;
                    } else {
                        pieChartRenderer = this;
                        double d2 = f17 * 0.017453292f;
                        f2 = rotationAngle;
                        pieChartRenderer.mPathBuffer.lineTo(mPPointF2.f1970x + (((float) Math.cos(d2)) * holeRadius), mPPointF2.f1971y + (holeRadius * ((float) Math.sin(d2))));
                ...
                if (f5 % f6 > Utils.FLOAT_EPSILON) {
                    if (z2) {
                        ...
                        pieChartRenderer.mPathBuffer.lineTo(mPPointF.f1970x + (((float) Math.cos(d3)) * fCalculateMinimumRadiusForSpacedSlice2), mPPointF.f1971y + (fCalculateMinimumRadiusForSpacedSlice2 * ((float) Math.sin(d3))));
                    } else {
                        pieChartRenderer.mPathBuffer.lineTo(mPPointF.f1970x, mPPointF.f1971y);
                    }
                }
                pieChartRenderer.mPathBuffer.close();
                pieChartRenderer.mBitmapCanvas.drawPath(pieChartRenderer.mPathBuffer, pieChartRenderer.mRenderPaint);
            ...
            if (zIsDrawValuesEnabled || zIsDrawEntryLabelsEnabled) {
                ...
                while (i7 < entryCount) {
                    ...
                    if (z || z2) {
                        ...
                        float fAbs = iPieDataSet3.isValueLineVariableLength() ? valueLinePart2Length * f10 * ((float) Math.abs(Math.sin(d))) : valueLinePart2Length * f10;
                        float f14 = (f3 * fCos) + centerCircleBox.f1970x;
                        float f15 = (f3 * fSin) + centerCircleBox.f1971y;
                        float f16 = (valueLinePart1Length + 1.0f) * f10;
                        valuePosition2 = xValuePosition;
                        float f17 = (f16 * fCos) + centerCircleBox.f1970x;
                        float f18 = (f16 * fSin) + centerCircleBox.f1971y;
                        double d2 = ((double) f11) % 360.0d;
                        ...
                        if (z && z2) {
                            ...
                            if (i2 < pieData.getEntryCount() && entryForIndex.getLabel() != null) {
                                drawEntryLabel(canvas, entryForIndex.getLabel(), f6, f18 + fCalcTextHeight);
                            }
                        } else {
                            iPieDataSet = iPieDataSet3;
                            f8 = fCos;
                            ...
                            if (z) {
                                if (i2 < pieData.getEntryCount() && entryForIndex.getLabel() != null) {
                                    drawEntryLabel(canvas, entryForIndex.getLabel(), f20, f18 + (fCalcTextHeight / 2.0f));
                                }
                            } else if (z2) {
                                iPieDataSet2 = iPieDataSet;
                                drawValue(canvas, valueFormatter, y, entryForIndex, 0, f20, f18 + (fCalcTextHeight / 2.0f), iPieDataSet2.getValueTextColor(i2));
                            }
                        }
                        iPieDataSet2 = iPieDataSet;
                    } else {
                    ...
                    if (z3 || z4) {
                        float f21 = (f10 * f8) + centerCircleBox.f1970x;
                        float f22 = (fSin * f10) + centerCircleBox.f1971y;
                        this.mValuePaint.setTextAlign(Paint.Align.CENTER);
                        if (z3 && z4) {
                            fArr3 = absoluteAngles;
                            drawValue(canvas, valueFormatter, y, entryForIndex, 0, f21, f22, iPieDataSet2.getValueTextColor(i2));
                            if (i2 < pieData.getEntryCount() && entryForIndex.getLabel() != null) {
                                drawEntryLabel(canvas, entryForIndex.getLabel(), f21, f22 + fCalcTextHeight);
                            }
                        } else {
                            fArr3 = absoluteAngles;
                            if (z3) {
                                if (i2 < pieData.getEntryCount() && entryForIndex.getLabel() != null) {
                                    drawEntryLabel(canvas, entryForIndex.getLabel(), f21, f22 + (fCalcTextHeight / 2.0f));
                                }
                            } else if (z4) {
                                drawValue(canvas, valueFormatter, y, entryForIndex, 0, f21, f22 + (fCalcTextHeight / 2.0f), iPieDataSet2.getValueTextColor(i2));
                            }
                        }
                    } else {
                        fArr3 = absoluteAngles;
    ...
    protected void drawHole(Canvas canvas) {
        ...
        if (Color.alpha(this.mHolePaint.getColor()) > 0) {
            this.mBitmapCanvas.drawCircle(centerCircleBox.f1970x, centerCircleBox.f1971y, holeRadius, this.mHolePaint);
        }
        if (Color.alpha(this.mTransparentCirclePaint.getColor()) > 0 && this.mChart.getTransparentCircleRadius() > this.mChart.getHoleRadius()) {
            ...
            this.mHoleCirclePath.addCircle(centerCircleBox.f1970x, centerCircleBox.f1971y, transparentCircleRadius, Path.Direction.CW);
            this.mHoleCirclePath.addCircle(centerCircleBox.f1970x, centerCircleBox.f1971y, holeRadius, Path.Direction.CCW);
    ...
    protected void drawCenterText(Canvas canvas) {
        ...
        float f = centerCircleBox.f1970x + centerTextOffset.f1970x;
        float f2 = centerCircleBox.f1971y + centerTextOffset.f1971y;
    ...
    public void drawHighlighted(Canvas canvas, Highlight[] highlightArr) {
        ...
        while (i5 < highlightArr2.length) {
            ...
            if (x < drawAngles.length && (dataSetByIndex = ((PieData) this.mChart.getData()).getDataSetByIndex(highlightArr2[i5].getDataSetIndex())) != null && dataSetByIndex.isHighlightEnabled()) {
                ...
                rectF2.inset(f9, f9);
                boolean z2 = sliceSpace > 0.0f && f7 <= 180.0f;
                this.mRenderPaint.setColor(dataSetByIndex.getColor(x));
                float f10 = i6 == 1 ? 0.0f : sliceSpace / (radius * 0.017453292f);
                float f11 = i6 == 1 ? 0.0f : sliceSpace / (f8 * 0.017453292f);
                float f12 = rotationAngle + ((f3 + (f10 / 2.0f)) * phaseY);
                float f13 = (f7 - f10) * phaseY;
                float f14 = f13 < 0.0f ? 0.0f : f13;
                float f15 = ((f3 + (f11 / 2.0f)) * phaseY) + rotationAngle;
                float f16 = (f7 - f11) * phaseY;
                if (f16 < 0.0f) {
                    f16 = 0.0f;
                }
                this.mPathBuffer.reset();
                if (f14 >= 360.0f && f14 % 360.0f <= Utils.FLOAT_EPSILON) {
                    this.mPathBuffer.addCircle(centerCircleBox.f1970x, centerCircleBox.f1971y, f8, Path.Direction.CW);
                    f4 = holeRadius;
                    f2 = phaseX;
                    fArr = drawAngles;
                    fArr2 = absoluteAngles;
                    ...
                    fArr = drawAngles;
                    fArr2 = absoluteAngles;
                    this.mPathBuffer.moveTo(centerCircleBox.f1970x + (((float) Math.cos(d)) * f8), centerCircleBox.f1971y + (f8 * ((float) Math.sin(d))));
                ...
                if (z2) {
                    ...
                    rectF = rectF2;
                    i4 = 1;
                    fCalculateMinimumRadiusForSpacedSlice = calculateMinimumRadiusForSpacedSlice(centerCircleBox, radius, f7 * phaseY, (((float) Math.cos(d2)) * radius) + centerCircleBox.f1970x, centerCircleBox.f1971y + (((float) Math.sin(d2)) * radius), f12, f14);
                } else {
                    f5 = f4;
                    rectF = rectF2;
                    i3 = i6;
                ...
                this.mInnerRectBuffer.set(centerCircleBox.f1970x - f5, centerCircleBox.f1971y - f5, centerCircleBox.f1970x + f5, centerCircleBox.f1971y + f5);
                if (!z || (f5 <= f6 && !z2)) {
                    f = f5;
                    if (f14 % 360.0f > Utils.FLOAT_EPSILON) {
                        if (z2) {
                            double d3 = (f12 + (f14 / 2.0f)) * 0.017453292f;
                            this.mPathBuffer.lineTo(centerCircleBox.f1970x + (((float) Math.cos(d3)) * fCalculateMinimumRadiusForSpacedSlice), centerCircleBox.f1971y + (fCalculateMinimumRadiusForSpacedSlice * ((float) Math.sin(d3))));
                        } else {
                            this.mPathBuffer.lineTo(centerCircleBox.f1970x, centerCircleBox.f1971y);
                        }
                    }
                } else {
                    if (z2) {
                        if (fCalculateMinimumRadiusForSpacedSlice < f6) {
                            fCalculateMinimumRadiusForSpacedSlice = -fCalculateMinimumRadiusForSpacedSlice;
                        }
                    ...
                    float f17 = (i3 == i4 || fMax == f6) ? 0.0f : sliceSpace / (fMax * 0.017453292f);
                    float f18 = rotationAngle + ((f3 + (f17 / 2.0f)) * phaseY);
                    ...
                    float f20 = f18 + f19;
                    if (f14 >= 360.0f && f14 % 360.0f <= Utils.FLOAT_EPSILON) {
                        this.mPathBuffer.addCircle(centerCircleBox.f1970x, centerCircleBox.f1971y, fMax, Path.Direction.CCW);
                        f = f5;
                    } else {
                        double d4 = f20 * 0.017453292f;
                        f = f5;
                        this.mPathBuffer.lineTo(centerCircleBox.f1970x + (((float) Math.cos(d4)) * fMax), centerCircleBox.f1971y + (fMax * ((float) Math.sin(d4))));
            ...
            if (dataSet.isVisible()) {
                ...
                while (i < dataSet.getEntryCount()) {
                    ...
                    if (Math.abs(dataSet.getEntryForIndex(i).getY()) > Utils.FLOAT_EPSILON) {
                        ...
                        fArr = drawAngles;
                        f2 = rotationAngle;
                        float fCos = (float) (((double) centerCircleBox.f1970x) + (Math.cos(Math.toRadians(d2)) * d));
                        float fSin = (float) ((d * Math.sin(Math.toRadians(d2))) + ((double) centerCircleBox.f1971y));
