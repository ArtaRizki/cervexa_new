    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        if (!this.mGestureDetector.onTouchEvent(motionEvent) && ((PieRadarChartBase) this.mChart).isRotationEnabled()) {
            ...
            if (action == 0) {
                ...
                this.mTouchStartPoint.f1970x = x;
                this.mTouchStartPoint.f1971y = y;
            } else if (action == 1) {
                if (((PieRadarChartBase) this.mChart).isDragDecelerationEnabled()) {
                    stopDeceleration();
                    sampleVelocity(x, y);
                ...
                if (this.mTouchMode == 0 && distance(x, this.mTouchStartPoint.f1970x, y, this.mTouchStartPoint.f1971y) > Utils.convertDpToPixel(8.0f)) {
