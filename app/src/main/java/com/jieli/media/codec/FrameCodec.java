...
public class FrameCodec extends AbstractCodec {
    ...
    protected void onCodecSuccess(final byte[] bArr, int i, int i2, int i3, String str) {
        ...
        for (final OnFrameCodecListener onFrameCodecListener : this.mOnFrameCodecListeners) {
            this.mHandler.post(new Runnable() { // from class: com.jieli.media.codec.FrameCodec.1
                @Override // java.lang.Runnable
                public void run() {
                    OnFrameCodecListener onFrameCodecListener2 = onFrameCodecListener;
                    if (onFrameCodecListener2 != null) {
                        onFrameCodecListener2.onCompleted(bArr, mediaMeta);
                    } else {
        ...
        for (final OnFrameCodecListener onFrameCodecListener : this.mOnFrameCodecListeners) {
            this.mHandler.post(new Runnable() { // from class: com.jieli.media.codec.FrameCodec.2
                ...
                public void run() {
                    OnFrameCodecListener onFrameCodecListener2 = onFrameCodecListener;
                    if (onFrameCodecListener2 != null) {
                        onFrameCodecListener2.onError(str);
                    }
                }
            });
        }
