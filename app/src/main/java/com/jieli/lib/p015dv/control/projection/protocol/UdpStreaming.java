package com.jieli.lib.p015dv.control.projection.protocol;

import android.text.TextUtils;
import com.jieli.lib.p015dv.control.projection.OnSendStateListener;
import com.jieli.lib.p015dv.control.projection.beans.StreamData;
import com.jieli.lib.p015dv.control.projection.tools.Utils;
import com.jieli.lib.p015dv.control.utils.Dlog;
import java.io.IOException;
import java.net.DatagramPacket;
import java.net.DatagramSocket;
import java.net.InetAddress;
import java.net.SocketException;
import java.net.UnknownHostException;
import java.util.Calendar;

/* JADX INFO: loaded from: classes.dex */
public final class UdpStreaming extends BaseStreaming {

    /* JADX INFO: renamed from: a */
    private String f2190a = getClass().getSimpleName();

    /* JADX INFO: renamed from: b */
    private int f2191b = 0;

    /* JADX INFO: renamed from: c */
    private int f2192c = 0;

    /* JADX INFO: renamed from: d */
    private DatagramSocket f2193d;

    /* JADX INFO: renamed from: e */
    private InetAddress f2194e;

    /* JADX INFO: renamed from: f */
    private int f2195f;

    /* JADX INFO: renamed from: g */
    private OnSendStateListener f2196g;

    @Override // com.jieli.lib.p015dv.control.projection.IPushStream
    public void create(String str) {
        create(str, 2230);
    }

    @Override // com.jieli.lib.p015dv.control.projection.IPushStream
    public void create(String str, int i) {
        this.f2195f = i;
        if (TextUtils.isEmpty(str)) {
            throw new NullPointerException("Destination IP is null");
        }
        try {
            this.f2193d = new DatagramSocket();
        } catch (SocketException e) {
            e.printStackTrace();
        }
        try {
            this.f2194e = InetAddress.getByName(str);
        } catch (UnknownHostException e2) {
            e2.printStackTrace();
        }
    }

    @Override // com.jieli.lib.p015dv.control.projection.IPushStream
    public void close() {
        synchronized (this) {
            if (this.f2193d != null) {
                this.f2193d.disconnect();
                this.f2193d.close();
                this.f2193d = null;
            }
            if (this.f2194e != null) {
                this.f2194e = null;
            }
            this.f2196g = null;
        }
    }

    @Override // com.jieli.lib.p015dv.control.projection.IPushStream
    public boolean send(int i, byte[] bArr) {
        if (m1376a(i, bArr)) {
            OnSendStateListener onSendStateListener = this.f2196g;
            if (onSendStateListener != null) {
                onSendStateListener.onState(1, "Successful");
            }
            return true;
        }
        OnSendStateListener onSendStateListener2 = this.f2196g;
        if (onSendStateListener2 == null) {
            return false;
        }
        onSendStateListener2.onState(-1, "Failed: data incorrect");
        return false;
    }

    @Override // com.jieli.lib.p015dv.control.projection.IPushStream
    public void setOnSendStateListener(OnSendStateListener onSendStateListener) {
        this.f2196g = onSendStateListener;
    }

    /* JADX INFO: renamed from: a */
    private void m1375a(byte[] bArr) {
        if (bArr != null && bArr.length > 0) {
            synchronized (this) {
                try {
                    DatagramPacket datagramPacket = new DatagramPacket(bArr, bArr.length, this.f2194e, this.f2195f);
                    if (this.f2193d != null) {
                        this.f2193d.send(datagramPacket);
                    } else if (this.f2196g != null) {
                        this.f2196g.onState(-1, "Socket was destroy");
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                    if (this.f2196g != null) {
                        this.f2196g.onState(-1, e.getMessage());
                    }
                }
            }
            return;
        }
        Dlog.m1383e(this.f2190a, "data maybe is null");
    }

    /* JADX INFO: renamed from: a */
    private boolean m1376a(int i, byte[] bArr) {
        int i2;
        int i3;
        byte[] bArr2;
        if (bArr != null && bArr.length > 0) {
            int length = bArr.length;
            if (length % 1451 == 0) {
                i2 = length / 1451;
            } else {
                i2 = (length / 1451) + 1;
            }
            if (i == 2 || i == 3) {
                i3 = this.f2192c + 1;
                this.f2192c = i3;
            } else if (i == 1) {
                i3 = this.f2191b + 1;
                this.f2191b = i3;
            } else {
                i3 = 0;
            }
            if (i3 > 0) {
                for (int i4 = 0; i4 < i2; i4++) {
                    int i5 = i4 * 1451;
                    int i6 = length - i5;
                    if (i6 >= 1451) {
                        bArr2 = new byte[1451];
                        System.arraycopy(bArr, i5, bArr2, 0, 1451);
                    } else {
                        byte[] bArr3 = new byte[i6];
                        System.arraycopy(bArr, i5, bArr3, 0, i6);
                        bArr2 = bArr3;
                    }
                    StreamData streamData = new StreamData();
                    streamData.setType(i);
                    streamData.setSave(0);
                    streamData.setSeq(i3);
                    streamData.setFrameSize(length);
                    streamData.setOffset(i5);
                    streamData.setPayload(bArr2);
                    streamData.setPayloadLen(bArr2.length);
                    streamData.setDateFlag((int) Calendar.getInstance().getTimeInMillis());
                    byte[] bArrMergeDataPacket = Utils.mergeDataPacket(streamData);
                    if (bArrMergeDataPacket != null) {
                        m1375a(bArrMergeDataPacket);
                    }
                }
                return true;
            }
        }
        return false;
    }
}
