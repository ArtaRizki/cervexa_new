        ...
        if (datagramSocket != null) {
            ...
            if (length > 0) {
                ...
                if (data != null && data.length > 0 && length < data.length) {
                    ...
                    NotifyInfo cmdData = parseCmdData(bArr);
                    if (this.mHandlerWeakReference.get() != null) {
                        Message messageObtain = Message.obtain();
                        messageObtain.what = 101;
                        messageObtain.obj = cmdData;
                        this.mHandlerWeakReference.get().sendMessage(messageObtain);
                        return true;
                    }
                    Dlog.m1386w(tag, "mHandler is null or error.");
                    return true;
                }
                Dlog.m1386w(tag, "receive message is null or error.");
                return true;
            }
            Dlog.m1386w(tag, "tempBuf is null or error.");
            return true;
        }
        Dlog.m1386w(tag, "udpSocket is null");
        stopRunning();
        return false;
    }
