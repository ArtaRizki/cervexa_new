package com.jieli.lib.p015dv.control.receiver.task;
        ...
        if (bArr != null) {
            if (bArr.length >= 6) {
                if (bArr[0] == 67 && bArr[1] == 84 && bArr[2] == 80 && bArr[3] == 58) {
                    ...
                    ByteBuffer byteBufferOrder = ByteBuffer.wrap(bArr, 4, 2).order(ByteOrder.nativeOrder());
                    ...
                    int i3 = 6 + i2;
                    if (bArr.length >= 4) {
                        byteBufferOrder = ByteBuffer.wrap(bArr, i3, 4).order(ByteOrder.nativeOrder());
                    ...
                    if (this.mHandlerWeakReference.get() != null) {
                        ...
                        messageObtain.what = ClientImpl.MSG_RECEIVED_CTP_DATA;
                        messageObtain.obj = notifyInfo;
                        this.mHandlerWeakReference.get().sendMessage(messageObtain);
                    }
                    byteBufferOrder.clear();
                    return;
                }
                Dlog.m1383e(tag, "'CTP:' signature not match");
                return;
            }
        }
        Dlog.m1383e(tag, "onDataReceiver:buffer == null || buffer.length <6");
    }

    @Override // java.lang.Thread, java.lang.Runnable
    public void run() {
        super.run();
        Dlog.m1384i(tag, "Receiver thread is running...");
            ...
            if (socket != null && !socket.isClosed()) {
                try {
                    ...
                    if (inputStream != null && inputStream.available() > 0) {
                        ...
                        if (inputStream.read(bArr) == 4 && "CTP:".equals(new String(bArr))) {
                            ...
                            if (inputStream.read(bArr2) == 2) {
                                int iByteArrayToInt = BufChangeHex.byteArrayToInt(bArr2);
                                byte[] bArr3 = new byte[iByteArrayToInt];
                                if (inputStream.read(bArr3) == iByteArrayToInt) {
                                    String str = new String(bArr3);
                                    if (!TextUtils.isEmpty(str)) {
                                        notifyInfo.setTopic(str);
                                        byte[] bArr4 = new byte[4];
                                        if (inputStream.read(bArr4) == 4) {
                                            int iByteArrayToInt2 = BufChangeHex.byteArrayToInt(bArr4);
                                            if (iByteArrayToInt2 > 0 && iByteArrayToInt2 < 5242880) {
                                                byte[] bArr5 = new byte[iByteArrayToInt2];
                                                if (inputStream.read(bArr5) == iByteArrayToInt2) {
                                                    ...
                                                    if (!TextUtils.isEmpty(str2)) {
                                                        try {
                                                            ...
                                                            if (!jSONObject.isNull(Constants.JSON_PARAM)) {
                                                                ...
                                                                Dlog.m1386w(tag, "JSON_PARAM is null");
                                                            }
                                                        } catch (JSONException e) {
                                                            e.printStackTrace();
                                                        }
                                                    } else {
                                                        Dlog.m1386w(tag, "Payload empty");
                                                    }
                                                } else {
                                                    Dlog.m1386w(tag, "Payload len error");
                                                }
                                            } else {
                                                notifyInfo.setParams(null);
                                                notifyInfo.setErrorType(0);
                                            ...
                                            Dlog.m1386w(tag, "Content len error");
                                        }
                                    } else {
                                        Dlog.m1386w(tag, "topic is empty");
                                    }
                                } else {
                                    Dlog.m1386w(tag, "Read topic len error");
                                }
                            } else {
                                Dlog.m1386w(tag, "Topic len error");
                            }
                        } else {
                            Dlog.m1386w(tag, "CTP content error");
                        }
                    }
                    ...
                    if (i > 5) {
                        ...
                        Dlog.m1386w(tag, "Receiver error time:" + this.mReceiveErrorTime);
                        return;
                    }
                }
            } else {
