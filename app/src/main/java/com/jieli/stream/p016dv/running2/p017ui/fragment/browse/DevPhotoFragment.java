...
public class DevPhotoFragment extends BaseFragment implements View.OnClickListener, TimeLineAdapter.OnSubViewItemClickListener, BaseFooterView.OnLoadListener, OnNotifyListener {
        ...
        public boolean handleMessage(Message message) {
            if (DevPhotoFragment.this.getActivity() != null && message != null) {
                ...
                if (i == 1) {
                    ...
                    int i2 = message.arg1;
                    Dbug.m1389i(DevPhotoFragment.this.TAG, "download file result : " + i2);
                    if (i2 == 0) {
        ...
        JSonManager.getInstance().parseJSonData(str, new OnCompletedListener<Boolean>() { // from class: com.jieli.stream.dv.running2.ui.fragment.browse.DevPhotoFragment.2
            ...
            public void onCompleted(Boolean bool) {
                if (bool.booleanValue()) {
                    ...
                    Dbug.m1388e(DevPhotoFragment.this.TAG, "-tryToParseData- parseJSonData photoInfoList is null!!!");
                    return;
                }
                Dbug.m1388e(DevPhotoFragment.this.TAG, "-tryToParseData- parseJSonData failed!!!");
            }
        });
    }
    ...
    public void loadMoreData(int i, int i2) {
        ...
        Dbug.m1391w(this.TAG, "-loadMoreData- total size : " + size + ", startOffset = " + i);
        int i3 = size - i;
        this.emptyView.setVisibility(8);
        if (i3 <= 0) {
            if (i3 == 0) {
        ...
        Dbug.m1391w(this.TAG, "-loadMoreData- dataList size : " + this.dataList.size());
    ...
    public void handlerTaskList(boolean z) {
        ...
        if (list != null) {
            ...
            if (z) {
                if (size > 0) {
                    FileInfo fileInfoRemove = this.selectedList.remove(0);
                    if (this.fileOp == 164) {
        ...
        if (notifyInfo != null) {
            ...
            if (errorType == 0) {
                ...
                if (b == 2) {
                    ...
                    Dbug.m1391w(this.TAG, "-PHOTO_CTRL- photoDesc = " + strReplaceAll);
                    FileInfo fileInfo = JSonManager.parseFileInfo(strReplaceAll);
                    if (fileInfo != null) {
                        if (("1".equals(fileInfo.getCameraType()) ? 2 : 1) == this.mApplication.getDeviceSettingInfo().getCameraType()) {
                            if (this.photoInfoList == null) {
                                this.photoInfoList = new ArrayList();
                            }
                            this.photoInfoList.add(0, fileInfo);
                ...
                if (b == 3 && notifyInfo.getParams() != null) {
                    String str2 = notifyInfo.getParams().get(TopicKey.PATH);
                    if (TextUtils.isEmpty(str2)) {
                        return;
                    }
                    Dbug.m1391w(this.TAG, "-FILES_DELETE- delPath = " + str2);
                    FileInfo fileInfoFindFileInfo = findFileInfo(str2);
                    if (fileInfoFindFileInfo == null || (list = this.photoInfoList) == null || !list.remove(fileInfoFindFileInfo)) {
                        return;
                    }
                    List<FileInfo> list2 = this.selectedList;
                    if (list2 != null) {
                        list2.remove(fileInfoFindFileInfo);
                        setSelectNum(this.selectedList.size());
                    }
                    TimeLineAdapter timeLineAdapter2 = this.mAdapter;
