package com.jieli.stream.p016dv.running2.p017ui.adapter;
...
public class SubGridViewAdapter extends BaseAdapter implements IConstant {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (!this.gridView.isMeasure() && (fileInfo = (FileInfo) getItem(i)) != null) {
            ...
            if (!TextUtils.isEmpty(name)) {
                int iJudgeFileType = AppUtils.judgeFileType(name);
                if (fileInfo.getSource() == 1) {
                    path = fileInfo.getPath();
                } else {
                    path = AppUtils.splicingFilePath(this.mApplication.getAppName(), this.mApplication.getUUID(), AppUtils.checkCameraDir(fileInfo), IConstant.DIR_DOWNLOAD) + File.separator + downloadFilename;
                }
                if (iJudgeFileType == 1) {
                    ...
                    if (AppUtils.checkFileExist(path)) {
                        ...
                        viewHolder.ivThumb.setImageBitmap(ThumbLoader.getInstance().loadLocalThumbnail(this.mContext, path, this.viewWidth, this.viewHeight));
                    } else {
                        viewHolder.ivPicState.setVisibility(0);
                        viewHolder.ivThumb.setTag(path);
    ...
    private class LoadCover extends AsyncTask<Integer, Void, Bitmap> {
        ...
        public Bitmap doInBackground(Integer... numArr) {
            ...
            FileInfo fileInfo = (FileInfo) SubGridViewAdapter.this.getItem(iIntValue);
            this.info = fileInfo;
            Bitmap bitmap = null;
            if (fileInfo != null) {
                if (fileInfo.getSource() == 1) {
