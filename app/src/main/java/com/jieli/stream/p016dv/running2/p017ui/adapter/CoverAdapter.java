...
public class CoverAdapter extends RecyclerView.Adapter<CoverAdapter.ViewHolder> {
    ...
    public void addData(ThumbnailInfo thumbnailInfo) {
        if (thumbnailInfo == null || this.mDataList.contains(thumbnailInfo)) {
            Dbug.m1389i(this.tag, "add data failed. info : " + thumbnailInfo);
            return;
        }
    ...
    public void remove(int i) {
        ThumbnailInfo thumbnailInfoRemove;
        List<ThumbnailInfo> list = this.mDataList;
        if (list == null || i < 0 || i >= list.size() || (thumbnailInfoRemove = this.mDataList.remove(i)) == null) {
            return;
        }
        Dbug.m1389i(this.tag, "mDataList del file -> name= " + thumbnailInfoRemove.getName() + "   time=" + thumbnailInfoRemove.getCreateTime());
    }
