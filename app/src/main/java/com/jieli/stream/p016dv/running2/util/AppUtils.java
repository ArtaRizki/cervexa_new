...
public class AppUtils implements IConstant {
    ...
    public static boolean checkNetworkIsAvailable() {
        ...
        map.put(BaseFragment.DATA_NAME, "baidu.com");
        map.put("host", "111.13.101.208");
        map.put(IjkMediaPlayer.OnNativeInvokeListener.ARG_PORT, 80);
        arrayList.add(map);
        HashMap map2 = new HashMap();
        map2.put(BaseFragment.DATA_NAME, "baidu.com");
        map2.put("host", "123.125.114.144");
        map2.put(IjkMediaPlayer.OnNativeInvokeListener.ARG_PORT, 80);
        arrayList.add(map2);
        HashMap map3 = new HashMap();
        map3.put(BaseFragment.DATA_NAME, "baidu.com");
        map3.put("host", "180.149.132.47");
        map3.put(IjkMediaPlayer.OnNativeInvokeListener.ARG_PORT, 80);
        arrayList.add(map3);
        HashMap map4 = new HashMap();
        map4.put(BaseFragment.DATA_NAME, "baidu.com");
        map4.put("host", "220.181.57.217");
        map4.put(IjkMediaPlayer.OnNativeInvokeListener.ARG_PORT, 80);
        arrayList.add(map4);
        HashMap map5 = new HashMap();
        map5.put(BaseFragment.DATA_NAME, "qq.com");
        map5.put("host", "125.39.240.113");
        map5.put(IjkMediaPlayer.OnNativeInvokeListener.ARG_PORT, 80);
        arrayList.add(map5);
        HashMap map6 = new HashMap();
        map6.put(BaseFragment.DATA_NAME, "qq.com");
        map6.put("host", "61.135.157.156");
        map6.put(IjkMediaPlayer.OnNativeInvokeListener.ARG_PORT, 80);
        arrayList.add(map6);
        ...
        try {
            ...
            try {
                ...
                if (file.isDirectory()) {
                    ...
                    if (fileArrListFiles != null && file.length() > 0) {
                        HashMap map = new HashMap();
                        ...
                        while (i2 < length) {
                            ...
                            if (file2 == null || !file2.isFile()) {
                                ...
                                String str2 = simpleDateFormat.format(new Date(file2.lastModified()));
                                ...
                                map.put(file2.getName(), fileInfo2);
                        ...
                        if (arrayList6.size() > 0) {
                            ...
                            for (String str3 : arrayList6) {
                                if (!TextUtils.isEmpty(str3) && (fileInfo = (FileInfo) map.remove(str3)) != null) {
                    ...
                    String ymd_hms = TimeFormate.formatYMD_HMS(file.lastModified());
                    if (!TextUtils.isEmpty(ymd_hms)) {
                        ...
                        fileInfo3.setCreateTime(ymd_hms);
    ...
    public static List<FileInfo> queryAllLocalFileList(String str, String str2) {
        List<FileInfo> listQueryAllLocalFileList;
            ...
            if (str2.equals(file.getName())) {
                ...
                if (fileArrListFiles != null) {
                    for (File file2 : fileArrListFiles) {
                        if (file2.isDirectory() && (listQueryAllLocalFileList = queryAllLocalFileList(file2.getAbsolutePath(), str2)) != null && listQueryAllLocalFileList.size() > 0) {
                            arrayList.addAll(listQueryAllLocalFileList);
                        }
                    }
                }
            }
    ...
    public static List<ItemBean> convertDataList(List<FileInfo> list) {
        ...
        descSortWay(list);
        TreeSet<String> treeSet = new TreeSet(new Comparator<String>() { // from class: com.jieli.stream.dv.running2.util.AppUtils.3
            @Override // java.util.Comparator
            public int compare(String str, String str2) {
                if (str.compareTo(str2) > 0) {
                    return -1;
        ...
        while (it.hasNext()) {
            String strSubDateFlag = subDateFlag(it.next().getCreateTime());
            if (!TextUtils.isEmpty(strSubDateFlag)) {
                treeSet.add(strSubDateFlag);
            }
        }
        ArrayList arrayList = new ArrayList();
        if (treeSet.size() > 0) {
    ...
    public static String splicingFilePath(String str, String str2, String str3, String str4) {
        if (!TextUtils.isEmpty(str)) {
                ...
                for (String str6 : str.split(File.separator)) {
                    if (!TextUtils.isEmpty(str6)) {
                        ...
                        if (!file.exists() && file.mkdir()) {
                            Dbug.m1391w(TAG, "create root dir success! path : " + str5);
                        }
                    }
                }
            } else {
            ...
            if (!file3.exists() && file3.mkdir()) {
                Dbug.m1391w(TAG, "create one dir success!");
            }
            ...
            if (!file4.exists() && file4.mkdir()) {
                Dbug.m1391w(TAG, "create two dir success!");
            }
            ...
            if (!file5.exists() && file5.mkdir()) {
                Dbug.m1391w(TAG, "create three sub dir success!");
            }
            return str9;
        }
        return ROOT_PATH;
    ...
    public static void browseFileWithOther(BaseActivity baseActivity, String str, String str2) {
        if (baseActivity == null || str2 == null) {
            return;
        }
        try {
            Intent intent = new Intent("android.intent.action.VIEW");
            int iJudgeFileType = judgeFileType(str);
            if (iJudgeFileType != 1) {
                if (iJudgeFileType == 2) {
                    String str3 = "";
                    if (str.endsWith(".mov") || str.endsWith(".MOV")) {
                        ...
                        str3 = "avi";
                    } else if (str.endsWith(UVCCameraHelper.SUFFIX_MP4) || str.endsWith(".MP4")) {
                        str3 = "mp4";
                    }
                    if (Build.VERSION.SDK_INT >= 24) {
                        intent.setFlags(1);
                        intent.setDataAndType(FileProvider.getUriForFile(baseActivity, baseActivity.getApplication().getPackageName() + ".fileProvider", new File(str2)), "video/" + str3);
                    } else {
                        intent.setFlags(268435456);
                        intent.setDataAndType(Uri.parse("file://" + str2), "video/" + str3);
                    }
                } else {
                    Dbug.m1388e(TAG, "无法打开文件，路径：" + str2);
                }
            } else if (Build.VERSION.SDK_INT >= 24) {
                intent.setFlags(1);
                intent.setDataAndType(FileProvider.getUriForFile(baseActivity, baseActivity.getApplication().getPackageName() + ".fileProvider", new File(str2)), "image/*");
            } else {
                intent.setFlags(268435456);
            ...
            Dbug.m1388e(TAG, " error  " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    */
    ...
    public static void deleteFile(File file) {
        ...
        if (file.isFile()) {
            if (file.delete()) {
                Dbug.m1389i(TAG, "delete file success!");
                return;
            }
            return;
        }
        if (file.isDirectory()) {
            ...
            if (fileArrListFiles == null || fileArrListFiles.length == 0) {
                if (file.delete()) {
                    Dbug.m1389i(TAG, "delete empty file success!");
                    return;
                }
                return;
            }
    ...
    */
    ...
    /*
        ...
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    private static com.jieli.stream.p016dv.running2.bean.ServerInfo parseServerTxtInfo(java.lang.String r9, java.lang.String r10) {
        /*
    ...
    */
    ...
    public static Context changeAppLanguage(Context context, String str) {
        if (context == null) {
            Dbug.m1388e(TAG, "context is null");
            return null;
        }
        ...
        if (language != null) {
            Dbug.m1389i(TAG, "changeAppLanguage index=" + str + ", " + language);
            if (Build.VERSION.SDK_INT >= 24) {
            ...
            Dbug.m1388e(TAG, "No language match:" + str);
        }
        return context;
    }
    ...
    public static ArrayList<SDFileInfo> getFirmwareFile(String str) {
        File[] fileArrListFiles = new File(str).listFiles(new HiddenFilter());
        if (fileArrListFiles == null) {
            return null;
        }
        ArrayList<SDFileInfo> arrayList = new ArrayList<>();
    ...
    public static String createFilenameWidthTime(int i, String str) {
        ...
        if (i == 1) {
            ...
            if (i != 3) {
                Dbug.m1388e(TAG, "Invalid type=" + i);
    ...
    public static List<String> queryThumbDirPath(String str) {
        List<String> listQueryThumbDirPath;
        if (!TextUtils.isEmpty(str)) {
            File file = new File(str);
            if (file.exists()) {
                ArrayList arrayList = new ArrayList();
                ...
                for (File file2 : fileArrListFiles) {
                    if (file2.isDirectory() && (listQueryThumbDirPath = queryThumbDirPath(file2.getAbsolutePath())) != null && listQueryThumbDirPath.size() > 0) {
                        arrayList.addAll(listQueryThumbDirPath);
                    }
                }
                return arrayList;
            }
    ...
    public static List<FileInfo> queryThumbInfoList(String str) {
        List<String> listQueryThumbDirPath;
        if (str == null || (listQueryThumbDirPath = queryThumbDirPath(str)) == null || listQueryThumbDirPath.size() <= 0) {
            return null;
        }
        ArrayList arrayList = new ArrayList();
        Iterator<String> it = listQueryThumbDirPath.iterator();
        while (it.hasNext()) {
            List<FileInfo> listQueryLocalFileList = queryLocalFileList(it.next());
            if (listQueryLocalFileList != null) {
                arrayList.addAll(listQueryLocalFileList);
            }
        }
        if (arrayList.size() <= 0) {
            return arrayList;
    ...
    public static String queryThumbPath(String str, String str2) {
        if (!TextUtils.isEmpty(str) && !TextUtils.isEmpty(str2)) {
            ...
            List<FileInfo> listQueryThumbInfoList = queryThumbInfoList(str2);
            if (listQueryThumbInfoList != null) {
                Iterator<FileInfo> it = listQueryThumbInfoList.iterator();
                while (it.hasNext()) {
    ...
    public static int parseThumbPathForDuration(String str) {
        ...
        try {
            return Integer.valueOf(strArrSplit[strArrSplit.length - 1]).intValue();
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
    ...
    public static int getRtsFormat() {
        ...
        Dbug.m1388e(TAG, "getRtsFormat: settingInfo is null");
        return 1;
    }
