...
public class PopupMenu {
    ...
    private final ViewTreeObserver.OnGlobalLayoutListener mLocationLayoutListener = new ViewTreeObserver.OnGlobalLayoutListener() { // from class: com.jieli.stream.dv.running2.ui.widget.PopupMenu.2
        ...
        public void onGlobalLayout() {
            ...
            PointF pointFCalculatePopupLocation = popupMenu.calculatePopupLocation(popupMenu.mGravity, PopupMenu.this.popupWindow, PopupMenu.this.parentView);
            popupWindow.setClippingEnabled(true);
            popupWindow.update((int) pointFCalculatePopupLocation.x, (int) pointFCalculatePopupLocation.y, popupWindow.getWidth(), popupWindow.getHeight());
            popupWindow.getContentView().requestLayout();
            Dbug.m1388e(PopupMenu.this.tag, "x " + pointFCalculatePopupLocation.x + ", y " + pointFCalculatePopupLocation.y + ", w " + popupWindow.getWidth() + ", h " + popupWindow.getHeight());
        }
    };
    ...
    public PopupMenu(Context context, Map<Integer, Integer> map) {
        ...
        if (context == null) {
            Dbug.m1388e(this.tag, "PopupMenu context is null!");
            return;
        }
    ...
    public void showAtLocation(View view, int i, int i2, int i3) {
        ...
        if (popupWindow == null) {
            Dbug.m1388e(this.tag, "PopupMenu popupWindow is null!");
            return;
        }
    ...
    public PointF calculatePopupLocation(int i, PopupWindow popupWindow, View view) {
        ...
        RectF rectFCalculateRectInWindow = calculateRectInWindow(view);
        PointF pointF2 = new PointF(rectFCalculateRectInWindow.centerX(), rectFCalculateRectInWindow.centerY());
        if (i == 17) {
            pointF.x = pointF2.x - (popupWindow.getContentView().getWidth() / 2.0f);
            pointF.y = pointF2.y - (popupWindow.getContentView().getHeight() / 2.0f);
        } else if (i == 48) {
            pointF.x = pointF2.x - (popupWindow.getContentView().getWidth() / 2.0f);
            pointF.y = (rectFCalculateRectInWindow.top - popupWindow.getContentView().getHeight()) - 0;
        } else if (i == 80) {
            pointF.x = pointF2.x - (popupWindow.getContentView().getWidth() / 2.0f);
            pointF.y = rectFCalculateRectInWindow.bottom + 0;
        } else if (i == 8388611) {
            pointF.x = (rectFCalculateRectInWindow.left - popupWindow.getContentView().getWidth()) - 0;
            pointF.y = pointF2.y - (popupWindow.getContentView().getHeight() / 2.0f);
        } else if (i == 8388613) {
            pointF.x = rectFCalculateRectInWindow.right + 0;
            pointF.y = pointF2.y - (popupWindow.getContentView().getHeight() / 2.0f);
        } else {
    ...
    private class PopupAdapter extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            if (num2 != null) {
                ...
                if (iIntValue != 0) {
                    viewHolder.imageView.setImageResource(iIntValue);
                }
            }
            return view;
        }
