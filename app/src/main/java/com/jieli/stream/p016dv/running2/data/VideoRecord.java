...
import com.jieli.lib.p015dv.control.player.AviWrapper;
import com.jieli.lib.p015dv.control.player.MovWrapper;
import com.jieli.lib.p015dv.control.player.OnAviWrapperListener;
...
public class VideoRecord {
    ...
    private final OnRecordListener onRecordListener = new OnRecordListener() { // from class: com.jieli.stream.dv.running2.data.VideoRecord.5
        ...
        public void onError(int i, String str) {
            Dbug.m1388e(VideoRecord.this.tag, "Code " + i + ", msg:" + str);
    ...
    public void prepare(boolean z, OnRecordStateListener onRecordStateListener) {
        ...
        if (mediaInfo != null) {
            ...
            if (TextUtils.isEmpty(path)) {
                Dbug.m1388e(this.tag, "Filename is null");
                return;
            } else if (path.endsWith("AVI") || path.endsWith("avi")) {
                zStartMovWrapper = startAviWrapper();
            } else {
                zStartMovWrapper = startMovWrapper();
            }
    ...
    private boolean startMovWrapper() {
        ...
        if (availableExternalMemorySize < MIN_STORAGE_LIMIT) {
            Dbug.m1388e(this.tag, "Not enough storage space");
        ...
        if (2 == deviceSettingInfo.getCameraType()) {
            ...
            if (mediaInfo == null) {
                if (!this.mMovWrapper.setFrameRate(deviceSettingInfo.getRearRate())) {
                    Dbug.m1388e(this.tag, "Set frame rate failed");
                }
                if (!this.mMovWrapper.setAudioTrack(deviceSettingInfo.getRearSampleRate(), 1, 16)) {
                    Dbug.m1388e(this.tag, "Set audio track failed");
                }
            } else {
        ...
        if (TextUtils.isEmpty(str)) {
            Dbug.m1388e(this.tag, "Output path is incorrect");
            dispatchErrorMessage("Output path is incorrect");
            return false;
        }
        Dbug.m1389i(this.tag, "output path " + str);
        this.mCurrentFilePath = str;
        if (this.mMovWrapper.create(str)) {
            return true;
        }
        Dbug.m1388e(this.tag, "Create MOV wrapper failed");
    ...
    public synchronized boolean write(int i, byte[] bArr) {
        ...
        if (this.mMovWrapper == null && this.mAviWrapper == null) {
            ...
            zWrite = this.mAviWrapper.write(i, bArr);
        } else {
            zWrite = this.mMovWrapper.write(i, bArr);
        }
        if (!zWrite) {
            Dbug.m1391w(this.tag, "write data failed.");
            close();
        }
        return zWrite;
    }
    ...
    public void close() {
        if (this.mAviWrapper == null && this.mMovWrapper == null) {
            Dbug.m1391w(this.tag, "mAviWrapper or mMovWrapper not init.");
        } else {
            AviWrapper aviWrapper = this.mAviWrapper;
            if (aviWrapper != null) {
                if (aviWrapper.isRecording()) {
                ...
                if (this.mMovWrapper.close()) {
                    ...
                    Dbug.m1388e(this.tag, "Mov close failed");
                }
                this.mMovWrapper = null;
            }
        }
