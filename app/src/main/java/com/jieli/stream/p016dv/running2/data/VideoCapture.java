...
public class VideoCapture {
    ...
    private final FrameCodec.OnFrameCodecListener mOnFrameCodecListener = new FrameCodec.OnFrameCodecListener() { // from class: com.jieli.stream.dv.running2.data.VideoCapture.3
        ...
        public void onCompleted(byte[] bArr, MediaMeta mediaMeta) {
            ...
            boolean zBytesToFile = (bArr == null || TextUtils.isEmpty(str)) ? false : AppUtils.bytesToFile(bArr, str);
            Dbug.m1391w(VideoCapture.this.tag, "result " + zBytesToFile + ", outPath " + str);
        }

        @Override // com.jieli.media.codec.FrameCodec.OnFrameCodecListener
        public void onError(String str) {
            Dbug.m1388e(VideoCapture.this.tag, "Codec error:" + str);
    ...
    public void capture(byte[] bArr) {
        ...
        if (MainApplication.getApplication().getDeviceDesc().getVideoType() == 0) {
            if (isReady) {
                isReady = false;
                String strSplicingFilePath = AppUtils.splicingFilePath("MergeCamera", "Media", "Photo", "Screen");
                String str = strSplicingFilePath + File.separator + AppUtils.getLocalPhotoName();
                if (TextUtils.isEmpty(str)) {
                    return;
                }
                AppUtils.bytesToFile(bArr, str);
                saveFile(str, this.width, this.height, strSplicingFilePath + File.separator + AppUtils.getLocalPhotoName());
                return;
            }
            return;
        }
