...
import com.jieli.lib.p015dv.control.player.MovWrapper;
import com.jieli.lib.p015dv.control.player.OnPlaybackListener;
import com.jieli.lib.p015dv.control.player.OnRecordListener;
import com.jieli.lib.p015dv.control.player.PlaybackStream;
import com.jieli.stream.p016dv.running2.bean.FileInfo;
import com.jieli.stream.p016dv.running2.interfaces.OnDownloadListener;
...
public class VideoManager {
    ...
    private OnRecordListener mOnRecordListener = new OnRecordListener() { // from class: com.jieli.stream.dv.running2.util.VideoManager.3
        @Override // com.jieli.lib.p015dv.control.player.OnRecordListener
        public void onError(int i, String str) {
            super.onError(i, str);
        ...
        public void onStateChanged(int i, String str) {
            ...
            Dbug.m1388e(VideoManager.tag, "-onStateChanged- state : " + i);
            if (i == 1) {
    ...
    public static FileInfo getDownloadFileInfo(long j, long j2) {
        if (j <= 0 || j2 <= 0 || j >= j2) {
            Dbug.m1388e(tag, "Illegal argument exception. start=" + j + ", end=" + j2);
            return null;
        }
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
        Dbug.m1389i(tag, "start:" + simpleDateFormat.format(Long.valueOf(j)) + ", end:" + simpleDateFormat.format(Long.valueOf(j2)));
        FileInfo selectedMedia = getSelectedMedia(j);
        if (selectedMedia == null) {
            Dbug.m1388e(tag, "Start VideoInfo not found.");
            return null;
        }
        ...
        Dbug.m1389i(tag, "download duration=" + downloadVideoDuration);
        if (downloadVideoDuration >= 0) {
            return fileInfo;
        }
        Dbug.m1388e(tag, "Total time error:" + downloadVideoDuration);
        return null;
    }
        ...
        if (j > 0 && !TextUtils.isEmpty(str)) {
            ...
            if (selectedMedia != null) {
                ...
                if (j2 > 0 && j2 > j) {
                    ...
                    Dbug.m1391w(tag, "-tryToDownload- duration : " + this.duration);
                ...
                Dbug.m1391w(tag, "-tryToDownload- offset : " + offset);
    ...
    private static synchronized FileInfo getSelectedMedia(long j) {
        if (j <= 0) {
            Dbug.m1388e(tag, "milliseconds <= 0: " + j);
            return null;
        }
        ...
        if (videoInfoList != null) {
            ...
            Dbug.m1391w(tag, "Can not find out milliseconds=" + j);
        }
        return null;
    }
    ...
    private static long getDownloadVideoDuration(long j, long j2) {
        ...
        if (videoInfoList != null) {
            ...
            Dbug.m1391w(tag, "Can not find out thumbnail at " + j);
        }
        return j3;
    }
