...
public class MapUtil {
    ...
    public void drawHistoryTrack(List<LatLng> list, SortType sortType) {
        ...
        MarkerOptions markerOptionsDraggable = new MarkerOptions().position(latLng).icon(BitmapUtil.bmStart).zIndex(9).draggable(true);
        PolylineOptions polylineOptionsPoints = new PolylineOptions().width(10).color(Color.rgb(30, IjkMediaMeta.FF_PROFILE_H264_HIGH_444, 255)).points(list);
        this.baiduMap.addOverlay(markerOptionsDraggable);
        this.polylineOverlay = this.baiduMap.addOverlay(polylineOptionsPoints);
    ...
    public void animateMapStatus(List<LatLng> list) {
        if (list == null || list.isEmpty()) {
            Dbug.m1388e(this.tag, "animateMapStatus:null error");
            return;
        }
        ...
        MapStatusUpdate mapStatusUpdateNewLatLngBounds = MapStatusUpdateFactory.newLatLngBounds(builder.build());
        BaiduMap baiduMap = this.baiduMap;
        if (baiduMap != null) {
            baiduMap.animateMapStatus(mapStatusUpdateNewLatLngBounds);
        }
    }

    public void animateMapStatus(LatLng latLng, float f) {
        MapStatus mapStatusBuild = new MapStatus.Builder().target(latLng).zoom(f).build();
        this.mapStatus = mapStatusBuild;
        BaiduMap baiduMap = this.baiduMap;
        if (baiduMap != null) {
            baiduMap.animateMapStatus(MapStatusUpdateFactory.newMapStatus(mapStatusBuild));
        }
    }
    ...
    public void setMapStatus(LatLng latLng, float f) {
        ...
        BaiduMap baiduMap = this.baiduMap;
        if (baiduMap != null) {
            baiduMap.setMapStatus(MapStatusUpdateFactory.newMapStatus(mapStatusBuild));
        }
    }
    ...
    public void addEndPointOverlay(List<LatLng> list) {
        ...
        MarkerOptions markerOptionsDraggable = new MarkerOptions().position(list.get(list.size() - 1)).icon(BitmapUtil.bmEnd).zIndex(9).draggable(true);
        BaiduMap baiduMap = this.baiduMap;
        if (baiduMap != null) {
            baiduMap.addOverlay(markerOptionsDraggable);
        }
    }
