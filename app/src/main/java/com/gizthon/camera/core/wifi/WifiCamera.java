...
public class WifiCamera implements ICamera {
    ...
    public void initDevice(Activity activity) {
        ...
        Log.i("TAG", "wifi 的IP地址 " + strIntToIp);
        Log.i("TAG", "wifi 的信息 " + connectionInfo.getSSID());
        String ssid = TextUtils.isEmpty(connectionInfo.getSSID()) ? "" : connectionInfo.getSSID();
        if (TextUtils.equals(strIntToIp, "192.168.1.2") && ssid.contains("wifi_camera")) {
            this.isWifi = true;
            this.isJieli = false;
        } else if (TextUtils.equals(strIntToIp, "192.168.25.101") && ssid.contains("Cam-")) {
            this.isWifi = true;
            this.isJieli = true;
        } else {
            this.isWifi = false;
