...
public class GalleryPhotoItemBindingImpl extends GalleryPhotoItemBinding implements OnLongClickListener.Listener, OnClickListener.Listener {
    ...
    private GalleryPhotoItemBindingImpl(DataBindingComponent dataBindingComponent, View view, Object[] objArr) {
        ...
        this.mboundView3androidCheckedAttrChanged = new InverseBindingListener() { // from class: com.gizthon.camera.databinding.GalleryPhotoItemBindingImpl.1
            ...
            public void onChange() {
                boolean zIsChecked = GalleryPhotoItemBindingImpl.this.mboundView3.isChecked();
                PhotoBean photoBean = GalleryPhotoItemBindingImpl.this.mViewModel;
                if (photoBean != null) {
                    photoBean.setSelected(zIsChecked);
                }
            }
        };
        this.mDirtyFlags = -1L;
    ...
    protected void executeBindings() {
        ...
        if ((61 & j) != 0) {
            ...
            selectedCoverVisible = ((j & 49) == 0 || photoBean == null) ? 0 : photoBean.getSelectedCoverVisible();
            if ((j & 41) == 0 || photoBean == null) {
                i = videoVisible;
                zIsSelected = false;
            } else {
                i = videoVisible;
                zIsSelected = photoBean.isSelected();
            }
        } else {
            zIsSelected = false;
            selectedCoverVisible = 0;
        }
        if ((32 & j) != 0) {
        ...
        if ((41 & j) != 0) {
            CompoundButtonBindingAdapter.setChecked(this.mboundView3, zIsSelected);
        }
        if ((j & 49) != 0) {
            this.mboundView3.setVisibility(selectedCoverVisible);
        }
    }
