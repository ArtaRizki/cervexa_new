...
class BottomBarBadge extends TextView {
    ...
    void setColoredCircleBackground(int i) {
        int iDpToPixel = MiscUtils.dpToPixel(getContext(), 1.0f);
        ShapeDrawable shapeDrawableMake = BadgeCircle.make(iDpToPixel * 3, i);
        setPadding(iDpToPixel, iDpToPixel, iDpToPixel, iDpToPixel);
        setBackgroundCompat(shapeDrawableMake);
    }
    ...
    void adjustPositionAndSize(BottomBarTab bottomBarTab) {
        ...
        if (layoutParams.width == iMax && layoutParams.height == iMax) {
            return;
        }
