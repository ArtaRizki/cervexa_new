...
public class BottomBar extends LinearLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    private void updateItems(List<BottomBarTab> list) {
        ...
        for (BottomBarTab bottomBarTab : list) {
            ...
            if (!this.isTabletMode) {
                if (bottomBarTab.getWidth() > width) {
                    width = bottomBarTab.getWidth();
                }
    ...
    private void resizeTabsToCorrectSizes(BottomBarTab[] bottomBarTabArr) {
        int iPixelToDp = MiscUtils.pixelToDp(getContext(), getWidth());
        if (iPixelToDp <= 0 || iPixelToDp > this.screenWidth) {
            iPixelToDp = this.screenWidth;
        }
        int iMin = Math.min(MiscUtils.dpToPixel(getContext(), iPixelToDp / bottomBarTabArr.length), this.maxFixedItemWidth);
        double d = iMin;
    ...
    public Parcelable onSaveInstanceState() {
        Bundle bundleSaveState = saveState();
        bundleSaveState.putParcelable("superstate", super.onSaveInstanceState());
    ...
    private void handleBackgroundColorChange(BottomBarTab bottomBarTab, boolean z) {
        ...
        animateBGColorChange(outerView, barColorWhenSelected);
    ...
    private void backgroundCircularRevealAnimation(View view, final int i) {
        Animator animatorCreateCircularReveal = ViewAnimationUtils.createCircularReveal(this.backgroundOverlay, (int) (ViewCompat.getX(view) + (view.getMeasuredWidth() / 2)), (this.isTabletMode ? (int) ViewCompat.getY(view) : 0) + (view.getMeasuredHeight() / 2), 0, this.isTabletMode ? this.outerContainer.getHeight() : this.outerContainer.getWidth());
        if (this.isTabletMode) {
            animatorCreateCircularReveal.setDuration(500L);
        }
        animatorCreateCircularReveal.addListener(new AnimatorListenerAdapter() { // from class: com.roughike.bottombar.BottomBar.9
    ...
    private void toggleShyVisibility(boolean z) {
        BottomNavigationBehavior bottomNavigationBehaviorFrom = BottomNavigationBehavior.from(this);
        if (bottomNavigationBehaviorFrom != null) {
            bottomNavigationBehaviorFrom.setHidden(this, z);
        }
    }
}
