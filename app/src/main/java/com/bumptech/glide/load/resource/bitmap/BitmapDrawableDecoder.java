...
public class BitmapDrawableDecoder<DataType> implements ResourceDecoder<DataType, BitmapDrawable> {
    ...
    public Resource<BitmapDrawable> decode(DataType datatype, int i, int i2, Options options) throws IOException {
        Resource<Bitmap> resourceDecode = this.decoder.decode(datatype, i, i2, options);
        if (resourceDecode == null) {
            return null;
        }
        return LazyBitmapDrawableResource.obtain(this.resources, this.bitmapPool, resourceDecode.get());
    }
}
