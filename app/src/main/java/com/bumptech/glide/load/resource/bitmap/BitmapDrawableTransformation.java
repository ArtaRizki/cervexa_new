...
public class BitmapDrawableTransformation implements Transformation<BitmapDrawable> {
    ...
    public Resource<BitmapDrawable> transform(Context context, Resource<BitmapDrawable> resource, int i, int i2) {
        BitmapResource bitmapResourceObtain = BitmapResource.obtain(resource.get().getBitmap(), Glide.get(context).getBitmapPool());
        Resource<Bitmap> resourceTransform = this.wrapped.transform(context, bitmapResourceObtain, i, i2);
        return resourceTransform.equals(bitmapResourceObtain) ? resource : LazyBitmapDrawableResource.obtain(context, resourceTransform.get());
    }
