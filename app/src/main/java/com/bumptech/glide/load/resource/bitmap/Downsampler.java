...
public final class Downsampler {
    ...
    private Bitmap decodeFromWrappedStreams(InputStream inputStream, BitmapFactory.Options options, DownsampleStrategy downsampleStrategy, DecodeFormat decodeFormat, int i, int i2, boolean z, DecodeCallbacks decodeCallbacks) throws IOException {
        ...
        if ((options.inSampleSize == 1 || z2) && shouldUsePool(inputStream)) {
            if (z && z2) {
                ...
                if (Log.isLoggable(str, 2)) {
                    Log.v(str, "Calculated target [" + iRound + "x" + iRound2 + "] for source [" + i3 + "x" + i4 + "], sampleSize: " + i7 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            }
        ...
        if (Log.isLoggable(str, 2)) {
            logDecode(i3, i4, str2, options, bitmapDecodeStream, i, i2);
        }
        Bitmap bitmapRotateImageExif = null;
        if (bitmapDecodeStream != null) {
            bitmapDecodeStream.setDensity(this.displayMetrics.densityDpi);
            bitmapRotateImageExif = TransformationUtils.rotateImageExif(this.bitmapPool, bitmapDecodeStream, orientation);
    ...
    static void calculateScaling(DownsampleStrategy downsampleStrategy, int i, int i2, int i3, int i4, int i5, BitmapFactory.Options options) {
        ...
        if (Log.isLoggable(TAG, 2)) {
            Log.v(TAG, "Calculate scaling, source: [" + i2 + "x" + i3 + "], target: [" + i4 + "x" + i5 + "], exact scale factor: " + scaleFactor + ", power of 2 sample size: " + iMax + ", adjusted scale factor: " + f + ", target density: " + options.inTargetDensity + ", density: " + options.inDensity);
        }
    }
