...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private static final class StreamReader implements Reader {
        ...
        private final InputStream f1919is;

        StreamReader(InputStream inputStream) {
            this.f1919is = inputStream;
        }

        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public int getUInt16() throws IOException {
            return ((this.f1919is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f1919is.read() & 255);
        }

        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public short getUInt8() throws IOException {
            return (short) (this.f1919is.read() & 255);
        }
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long jSkip = this.f1919is.skip(j2);
                if (jSkip <= 0) {
                    if (this.f1919is.read() == -1) {
                        break;
                    }
                    jSkip = 1;
                }
        ...
        public int read(byte[] bArr, int i) throws IOException {
            int i2 = i;
            while (i2 > 0) {
                int i3 = this.f1919is.read(bArr, i - i2, i2);
                if (i3 == -1) {
                    break;
                }
                i2 -= i3;
