...
public class LoadPath<Data, ResourceType, Transcode> {
    ...
    public Resource<Transcode> load(DataRewinder<Data> dataRewinder, Options options, int i, int i2, DecodePath.DecodeCallback<ResourceType> decodeCallback) throws GlideException {
        List<Exception> listAcquire = this.listPool.acquire();
        try {
            return loadWithExceptionList(dataRewinder, options, i, i2, decodeCallback, listAcquire);
        } finally {
            this.listPool.release(listAcquire);
        }
    }
