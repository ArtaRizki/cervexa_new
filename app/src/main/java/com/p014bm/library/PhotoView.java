...
public class PhotoView extends ImageView {
    ...
    public PhotoView(Context context) {
        ...
        this.mRotateListener = new OnRotateListener() { // from class: com.bm.library.PhotoView.1
            @Override // com.p014bm.library.OnRotateListener
    ...
    private void initBase() {
        if (this.hasDrawable && this.isKnowSize) {
            ...
            executeTranslate();
            switch (C08946.$SwitchMap$android$widget$ImageView$ScaleType[this.mScaleType.ordinal()]) {
                case 1:
                    initCenter();
                    break;
                case 2:
    ...
    /* JADX INFO: renamed from: com.bm.library.PhotoView$6 */
    static /* synthetic */ class C08946 {
    ...
    private class Transform implements Runnable {
        ...
        void withFling(float f, float f2) {
            ...
            this.mFlingScroller.fling(this.mLastFlingX, this.mLastFlingY, (int) f, (int) f2, i, i2, i3, i4, Math.abs(iAbs) < PhotoView.this.MAX_FLING_OVER_SCROLL * 2 ? 0 : PhotoView.this.MAX_FLING_OVER_SCROLL, Math.abs(iAbs2) < PhotoView.this.MAX_FLING_OVER_SCROLL * 2 ? 0 : PhotoView.this.MAX_FLING_OVER_SCROLL);
        }
        ...
        public void run() {
            ...
            if (this.mClipScroller.computeScrollOffset() || PhotoView.this.mClip != null) {
                ...
                PhotoView.this.mTmpMatrix.setScale(currX3, currY3, (PhotoView.this.mImgRect.left + PhotoView.this.mImgRect.right) / 2.0f, this.f1900C.calculateTop());
    ...
    public void animaFrom(Info info) {
        if (this.isInit) {
            reset();
            Info info2 = getInfo();
            float fWidth = info.mImgRect.width() / info2.mImgRect.width();
            float fHeight = info.mImgRect.height() / info2.mImgRect.height();
            if (fWidth >= fHeight) {
                fWidth = fHeight;
            }
            float fWidth2 = info.mRect.left + (info.mRect.width() / 2.0f);
            float fHeight2 = info.mRect.top + (info.mRect.height() / 2.0f);
            float fWidth3 = info2.mRect.left + (info2.mRect.width() / 2.0f);
            float fHeight3 = info2.mRect.top + (info2.mRect.height() / 2.0f);
            this.mAnimaMatrix.reset();
            float f = fWidth2 - fWidth3;
            float f2 = fHeight2 - fHeight3;
            this.mAnimaMatrix.postTranslate(f, f2);
            this.mAnimaMatrix.postScale(fWidth, fWidth, fWidth2, fHeight2);
            this.mAnimaMatrix.postRotate(info.mDegrees, fWidth2, fHeight2);
            executeTranslate();
            ...
            this.mTranslate.withScale(fWidth, 1.0f);
            ...
            if (info.mWidgetRect.width() < info.mImgRect.width() || info.mWidgetRect.height() < info.mImgRect.height()) {
                float fWidth4 = info.mWidgetRect.width() / info.mImgRect.width();
                float fHeight4 = info.mWidgetRect.height() / info.mImgRect.height();
                if (fWidth4 > 1.0f) {
                    fWidth4 = 1.0f;
                }
                if (fHeight4 > 1.0f) {
                ...
                this.mTranslate.withClip(fWidth4, fHeight4, 1.0f - fWidth4, 1.0f - fHeight4, this.mAnimaDuring / 3, start);
                this.mTmpMatrix.setScale(fWidth4, fHeight4, (this.mImgRect.left + this.mImgRect.right) / 2.0f, start.calculateTop());
    ...
    public void animaTo(Info info, Runnable runnable) {
        if (this.isInit) {
            ...
            this.mTranslateX = 0;
            this.mTranslateY = 0;
            float fWidth = info.mRect.left + (info.mRect.width() / 2.0f);
            float fHeight = info.mRect.top + (info.mRect.height() / 2.0f);
            ...
            this.mAnimaMatrix.mapRect(this.mImgRect, this.mBaseRect);
            float fWidth2 = info.mImgRect.width() / this.mBaseRect.width();
            float fHeight2 = info.mImgRect.height() / this.mBaseRect.height();
            if (fWidth2 <= fHeight2) {
                fWidth2 = fHeight2;
            }
            this.mAnimaMatrix.postRotate(this.mDegrees, this.mScaleCenter.x, this.mScaleCenter.y);
            ...
            this.mTranslate.withTranslate(0, 0, (int) (fWidth - this.mScaleCenter.x), (int) (fHeight - this.mScaleCenter.y));
            ...
            if (info.mWidgetRect.width() < info.mRect.width() || info.mWidgetRect.height() < info.mRect.height()) {
                ...
                postDelayed(new Runnable() { // from class: com.bm.library.PhotoView.5
                    @Override // java.lang.Runnable
                    public void run() {
                        PhotoView.this.mTranslate.withClip(1.0f, 1.0f, fWidth3 - 1.0f, fHeight3 - 1.0f, PhotoView.this.mAnimaDuring / 2, start);
                    }
                }, this.mAnimaDuring / 2);
            }
