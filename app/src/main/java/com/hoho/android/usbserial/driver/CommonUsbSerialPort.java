...
public abstract class CommonUsbSerialPort implements UsbSerialPort {
    ...
    protected int read(byte[] bArr, int i, boolean z) throws IOException {
        ...
        if (i != 0) {
            long jCurrentTimeMillis = z ? System.currentTimeMillis() + ((long) i) : 0L;
            iPosition = this.mConnection.bulkTransfer(this.mReadEndpoint, bArr, Math.min(bArr.length, 16384), i);
            if (iPosition == -1 && z && System.currentTimeMillis() < jCurrentTimeMillis) {
                testConnection();
            }
        } else {
    ...
    public int write(byte[] bArr, int i) throws IOException {
        ...
        while (i2 < bArr.length) {
            ...
            Log.d(TAG, "Wrote amt=" + iBulkTransfer + " attempted=" + iMin);
            i2 += iBulkTransfer;
        }
        return i2;
    }
