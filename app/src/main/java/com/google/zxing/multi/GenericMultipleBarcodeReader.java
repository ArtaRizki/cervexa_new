...
public final class GenericMultipleBarcodeReader implements MultipleBarcodeReader {
    ...
    private void doDecodeMultiple(BinaryBitmap binaryBitmap, Hashtable hashtable, Vector vector, int i, int i2) {
        ...
        float f2;
        try {
            Result resultDecode = this.delegate.decode(binaryBitmap, hashtable);
            int i3 = 0;
            while (true) {
                if (i3 >= vector.size()) {
                    z = false;
                    ...
                    if (((Result) vector.elementAt(i3)).getText().equals(resultDecode.getText())) {
                        z = true;
                        break;
                    }
                    i3++;
            ...
            vector.addElement(translateResultPoints(resultDecode, i, i2));
