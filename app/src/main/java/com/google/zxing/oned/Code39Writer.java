...
public final class Code39Writer extends UPCEANWriter {
    ...
    public byte[] encode(String str) {
        ...
        int iAppendPattern = appendPattern(bArr, 0, iArr, 1);
        int[] iArr2 = {1};
        int iAppendPattern2 = iAppendPattern + appendPattern(bArr, iAppendPattern, iArr2, 0);
        for (int i4 = length - 1; i4 >= 0; i4--) {
            toIntArray(Code39Reader.CHARACTER_ENCODINGS["0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-. *$/+%".indexOf(str.charAt(i4))], iArr);
            int iAppendPattern3 = iAppendPattern2 + appendPattern(bArr, iAppendPattern2, iArr, 1);
            iAppendPattern2 = iAppendPattern3 + appendPattern(bArr, iAppendPattern3, iArr2, 0);
        }
        toIntArray(Code39Reader.CHARACTER_ENCODINGS[39], iArr);
        appendPattern(bArr, iAppendPattern2, iArr, 1);
        return bArr;
    }
}
