...
public final class ITFReader extends OneDReader {
    ...
    private static final int f2036N = 1;

    /* JADX INFO: renamed from: W */
    private static final int f2037W = 3;
    ...
    private static int decodeDigit(int[] iArr) throws NotFoundException {
        ...
        for (int i3 = 0; i3 < length; i3++) {
            int iPatternMatchVariance = patternMatchVariance(iArr, PATTERNS[i3], MAX_INDIVIDUAL_VARIANCE);
            if (iPatternMatchVariance < i) {
                i2 = i3;
                i = iPatternMatchVariance;
            }
        }
        if (i2 >= 0) {
            return i2;
    ...
    public Result decodeRow(int i, BitArray bitArray, Hashtable hashtable) throws NotFoundException, FormatException {
        boolean z;
        int[] iArrDecodeStart = decodeStart(bitArray);
        int[] iArrDecodeEnd = decodeEnd(bitArray);
        StringBuffer stringBuffer = new StringBuffer(20);
        decodeMiddle(bitArray, iArrDecodeStart[1], iArrDecodeEnd[0], stringBuffer);
