    ...
    */
    ...
    public Result decodeRow(int i, BitArray bitArray, Hashtable hashtable) throws NotFoundException {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            try {
                Result resultDecodeRow = ((UPCEANReader) this.readers.elementAt(i2)).decodeRow(i, bitArray, iArrFindStartGuardPattern, hashtable);
                boolean z = BarcodeFormat.EAN_13.equals(resultDecodeRow.getBarcodeFormat()) && resultDecodeRow.getText().charAt(0) == '0';
                Vector vector = hashtable == null ? null : (Vector) hashtable.get(DecodeHintType.POSSIBLE_FORMATS);
                return (z && (vector == null || vector.contains(BarcodeFormat.UPC_A))) ? new Result(resultDecodeRow.getText().substring(1), null, resultDecodeRow.getResultPoints(), BarcodeFormat.UPC_A) : resultDecodeRow;
