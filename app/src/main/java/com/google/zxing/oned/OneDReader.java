    ...
    */
    ...
    public Result decode(BinaryBitmap binaryBitmap, Hashtable hashtable) throws NotFoundException, FormatException {
        try {
            ...
            Result resultDoDecode = doDecode(binaryBitmapRotateCounterClockwise, hashtable);
            Hashtable resultMetadata = resultDoDecode.getResultMetadata();
            int iIntValue = VerticalSeekBar.ROTATION_ANGLE_CW_270;
            if (resultMetadata != null && resultMetadata.containsKey(ResultMetadataType.ORIENTATION)) {
                iIntValue = (((Integer) resultMetadata.get(ResultMetadataType.ORIENTATION)).intValue() + VerticalSeekBar.ROTATION_ANGLE_CW_270) % 360;
            }
            resultDoDecode.putMetadata(ResultMetadataType.ORIENTATION, new Integer(iIntValue));
