...
public final class CodaBarReader extends OneDReader {
    ...
    public Result decodeRow(int i, BitArray bitArray, Hashtable hashtable) throws NotFoundException {
        ...
        int i3 = iArrFindAsteriskPattern[1];
        int size = bitArray.getSize();
        ...
        while (true) {
            ...
            if (i4 >= size) {
                ...
                char cCharAt = stringBuffer.charAt(0);
                if (!arrayContains(STARTEND_ENCODING, cCharAt)) {
                    throw NotFoundException.getNotFoundInstance();
                }
                int length = 1;
                while (length < stringBuffer.length()) {
                    if (stringBuffer.charAt(length) == cCharAt && (i2 = length + 1) != stringBuffer.length()) {
                ...
                float f = i;
                return new Result(stringBuffer.toString(), null, new ResultPoint[]{new ResultPoint((iArrFindAsteriskPattern[1] + iArrFindAsteriskPattern[0]) / 2.0f, f), new ResultPoint((i4 + i3) / 2.0f, f)}, BarcodeFormat.CODABAR);
            }
            i3 = i4;
        }
    }
