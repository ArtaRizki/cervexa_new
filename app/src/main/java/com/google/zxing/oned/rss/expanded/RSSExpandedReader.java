...
public final class RSSExpandedReader extends AbstractRSSReader {
    ...
    private void adjustOddEvenCounts(int i) throws NotFoundException {
        ...
        boolean z2;
        boolean z3;
        int iCount = count(this.oddCounts);
        int iCount2 = count(this.evenCounts);
    ...
    private static Result constructResult(Vector vector) throws NotFoundException {
        String information = AbstractExpandedDecoder.createDecoder(BitArrayBuilder.buildBitArray(vector)).parseInformation();
    ...
    Vector decodeRow2pairs(int i, BitArray bitArray) throws NotFoundException {
        while (true) {
            ExpandedPair expandedPairRetrieveNextPair = retrieveNextPair(bitArray, this.pairs, i);
            this.pairs.addElement(expandedPairRetrieveNextPair);
    ...
    ExpandedPair retrieveNextPair(BitArray bitArray, Vector vector, int i) throws NotFoundException {
        ...
        boolean z2 = true;
        do {
            findNextPair(bitArray, vector, nextSecondBar);
        ...
        boolean zCheckPairSequence = checkPairSequence(vector, foundFinderPattern);
