...
final class DecodedBitStreamParser {
    ...
    static DecoderResult decode(byte[] bArr) throws FormatException {
        ...
        int iDecodeAsciiSegment = 1;
        do {
            if (iDecodeAsciiSegment == 1) {
                iDecodeAsciiSegment = decodeAsciiSegment(bitSource, stringBuffer, stringBuffer2);
            } else {
                if (iDecodeAsciiSegment == 2) {
                    decodeC40Segment(bitSource, stringBuffer);
                } else if (iDecodeAsciiSegment == 3) {
                    decodeTextSegment(bitSource, stringBuffer);
                } else if (iDecodeAsciiSegment == 4) {
                    decodeAnsiX12Segment(bitSource, stringBuffer);
                } else if (iDecodeAsciiSegment == 5) {
                    decodeEdifactSegment(bitSource, stringBuffer);
                } else {
                    if (iDecodeAsciiSegment != 6) {
                        throw FormatException.getFormatInstance();
                    }
                    decodeBase256Segment(bitSource, stringBuffer, vector);
                }
                iDecodeAsciiSegment = 1;
            }
            if (iDecodeAsciiSegment == 0) {
                break;
            }
        ...
        return new DecoderResult(bArr, string, vector, null);
    }
    ...
    private static int decodeAsciiSegment(BitSource bitSource, StringBuffer stringBuffer, StringBuffer stringBuffer2) throws FormatException {
        ...
        do {
            ...
            if (bits <= 128) {
                ...
                stringBuffer.append((char) (bits - 1));
                return 1;
            }
            if (bits == 129) {
                return 0;
    ...
    private static void decodeBase256Segment(BitSource bitSource, StringBuffer stringBuffer, Vector vector) throws FormatException {
        int iUnrandomize255State = unrandomize255State(bitSource.readBits(8), 2);
        int i = 3;
        if (iUnrandomize255State == 0) {
            iUnrandomize255State = bitSource.available() / 8;
        } else if (iUnrandomize255State >= 250) {
            iUnrandomize255State = ((iUnrandomize255State - 249) * 250) + unrandomize255State(bitSource.readBits(8), 3);
            i = 4;
        }
        if (iUnrandomize255State < 0) {
            throw FormatException.getFormatInstance();
