...
class ClockFaceView extends RadialViewGroup implements ClockHandView.OnRotateListener {
    ...
    public ClockFaceView(Context context, AttributeSet attributeSet, int i) {
        ...
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C1123R.styleable.ClockFaceView, i, 0);
        Resources resources = getResources();
        this.textColor = typedArrayObtainStyledAttributes.getColor(C1123R.styleable.ClockFaceView_valueTextColor, ViewCompat.MEASURED_STATE_MASK);
        LayoutInflater.from(context).inflate(C1123R.layout.material_clockface_view, (ViewGroup) this, true);
        this.clockHandView = (ClockHandView) findViewById(C1123R.id.material_clock_hand);
        this.clockHandPadding = resources.getDimensionPixelSize(C1123R.dimen.material_clock_hand_padding);
        int color = MaterialColors.getColor(this, C1123R.attr.colorOnSurface);
        int color2 = MaterialColors.getColor(this, C1123R.attr.colorOnPrimary);
        this.gradientColors = new int[]{color2, color2, color};
        this.clockHandView.addOnRotateListener(this);
        setBackgroundColor(AppCompatResources.getColorStateList(context, C1123R.color.material_timepicker_clockface).getDefaultColor());
        ...
        this.valueAccessibilityDelegate = new AccessibilityDelegateCompat() { // from class: com.google.android.material.timepicker.ClockFaceView.2
            ...
            public void onInitializeAccessibilityNodeInfo(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
                ...
                int iIntValue = ((Integer) view.getTag(C1123R.id.material_value_index)).intValue();
                if (iIntValue > 0) {
    ...
    private void updateTextViews(int i) {
        ...
        for (int i2 = 0; i2 < Math.max(this.values.length, this.textViewPool.size()); i2++) {
            ...
            if (i2 >= this.values.length) {
                ...
                if (textView == null) {
                    textView = (TextView) layoutInflaterFrom.inflate(C1123R.layout.material_clockface_textview, (ViewGroup) this, false);
                    addView(textView);
                    this.textViewPool.put(i2, textView);
                }
                textView.setText(this.values[i2]);
                textView.setTag(C1123R.id.material_value_index, Integer.valueOf(i2));
