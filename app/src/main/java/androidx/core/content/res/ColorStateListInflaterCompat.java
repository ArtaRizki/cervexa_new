    ...
    private static ColorStateList inflate(Resources resources, XmlPullParser xmlPullParser, AttributeSet attributeSet, Resources.Theme theme) throws XmlPullParserException, IOException {
        ...
        while (true) {
            ...
            if (next == 2 && depth <= depth2 && xmlPullParser.getName().equals("item")) {
                TypedArray typedArrayObtainAttributes = obtainAttributes(resources, theme, attributeSet, C0186R.styleable.ColorStateListItem);
                int color = typedArrayObtainAttributes.getColor(C0186R.styleable.ColorStateListItem_android_color, -65281);
                float f = 1.0f;
                if (typedArrayObtainAttributes.hasValue(C0186R.styleable.ColorStateListItem_android_alpha)) {
                    f = typedArrayObtainAttributes.getFloat(C0186R.styleable.ColorStateListItem_android_alpha, 1.0f);
                } else if (typedArrayObtainAttributes.hasValue(C0186R.styleable.ColorStateListItem_alpha)) {
                    f = typedArrayObtainAttributes.getFloat(C0186R.styleable.ColorStateListItem_alpha, 1.0f);
                }
                ...
                for (int i4 = 0; i4 < attributeCount; i4++) {
                    ...
                    if (attributeNameResource != 16843173 && attributeNameResource != 16843551 && attributeNameResource != C0186R.attr.alpha) {
                        int i5 = i3 + 1;
