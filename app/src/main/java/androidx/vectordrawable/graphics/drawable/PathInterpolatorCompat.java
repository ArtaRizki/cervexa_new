...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f80mX = new float[iMin];
        this.f81mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f80mX[i2] = fArr[0];
            this.f81mY[i2] = fArr[1];
        }
        if (Math.abs(this.f80mX[0]) <= 1.0E-5d && Math.abs(this.f81mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f80mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f81mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < iMin) {
                    float[] fArr2 = this.f80mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f80mX[0]);
        sb.append(",");
        sb.append(this.f81mY[0]);
        sb.append(" end:");
        int i6 = iMin - 1;
        sb.append(this.f80mX[i6]);
        sb.append(",");
        sb.append(this.f81mY[i6]);
