...
public class NotificationCompat {
    ...
    public static class MediaStyle extends NotificationCompat.Style {
        ...
        RemoteViews generateContentView() {
            ...
            remoteViewsApplyStandardTemplate.removeAllViews(C0475R.id.media_actions);
            if (iMin > 0) {
                for (int i = 0; i < iMin; i++) {
                    if (i >= size) {
                        throw new IllegalArgumentException(String.format("setShowActionsInCompactView: action %d out of bounds (max %d)", Integer.valueOf(i), Integer.valueOf(size - 1)));
                    }
                    remoteViewsApplyStandardTemplate.addView(C0475R.id.media_actions, generateMediaActionButton(this.mBuilder.mActions.get(this.mActionsToShowInCompact[i])));
                }
            }
            if (this.mShowCancelButton) {
                remoteViewsApplyStandardTemplate.setViewVisibility(C0475R.id.end_padder, 8);
                remoteViewsApplyStandardTemplate.setViewVisibility(C0475R.id.cancel_action, 0);
                remoteViewsApplyStandardTemplate.setOnClickPendingIntent(C0475R.id.cancel_action, this.mCancelButtonIntent);
                remoteViewsApplyStandardTemplate.setInt(C0475R.id.cancel_action, "setAlpha", this.mBuilder.mContext.getResources().getInteger(C0475R.integer.cancel_button_image_alpha));
            } else {
                remoteViewsApplyStandardTemplate.setViewVisibility(C0475R.id.end_padder, 0);
                remoteViewsApplyStandardTemplate.setViewVisibility(C0475R.id.cancel_action, 8);
            }
        ...
        private RemoteViews generateMediaActionButton(NotificationCompat.Action action) {
            ...
            RemoteViews remoteViews = new RemoteViews(this.mBuilder.mContext.getPackageName(), C0475R.layout.notification_media_action);
            remoteViews.setImageViewResource(C0475R.id.action0, action.getIcon());
            if (!z) {
                remoteViews.setOnClickPendingIntent(C0475R.id.action0, action.getActionIntent());
            }
            if (Build.VERSION.SDK_INT >= 15) {
                remoteViews.setContentDescription(C0475R.id.action0, action.getTitle());
            }
            return remoteViews;
        }

        int getContentViewLayoutResource() {
            return C0475R.layout.notification_template_media;
        }
        ...
        RemoteViews generateBigContentView() {
            ...
            if (iMin > 0) {
                for (int i = 0; i < iMin; i++) {
                    remoteViewsApplyStandardTemplate.addView(C0475R.id.media_actions, generateMediaActionButton(this.mBuilder.mActions.get(i)));
                }
            }
            if (this.mShowCancelButton) {
                remoteViewsApplyStandardTemplate.setViewVisibility(C0475R.id.cancel_action, 0);
        ...
        int getBigContentViewLayoutResource(int i) {
            return i <= 3 ? C0475R.layout.notification_template_big_media_narrow : C0475R.layout.notification_template_big_media;
        }
    }
    ...
    public static class DecoratedMediaCustomViewStyle extends MediaStyle {
        ...
        int getContentViewLayoutResource() {
            return this.mBuilder.getContentView() != null ? C0475R.layout.notification_template_media_custom : super.getContentViewLayoutResource();
        }
        ...
        @Override // androidx.media.app.NotificationCompat.MediaStyle
        int getBigContentViewLayoutResource(int i) {
            return i <= 3 ? C0475R.layout.notification_template_big_media_narrow_custom : C0475R.layout.notification_template_big_media_custom;
        }
        ...
        private void setBackgroundColor(RemoteViews remoteViews) {
            ...
            if (this.mBuilder.getColor() != 0) {
                ...
                color = this.mBuilder.mContext.getResources().getColor(C0475R.color.notification_material_background_media_default_color);
            }
            remoteViews.setInt(C0475R.id.status_bar_latest_event_content, "setBackgroundColor", color);
        }
    }
}
