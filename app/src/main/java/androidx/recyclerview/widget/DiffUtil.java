...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f71x - snake2.f71x;
            return i == 0 ? snake.f72y - snake2.f72y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (snakeDiffPartial != null) {
                ...
                snakeDiffPartial.f71x += range.oldListStart;
                snakeDiffPartial.f72y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (snakeDiffPartial.reverse) {
                    range2.oldListEnd = snakeDiffPartial.f71x;
                    range2.newListEnd = snakeDiffPartial.f72y;
                } else if (snakeDiffPartial.removal) {
                    range2.oldListEnd = snakeDiffPartial.f71x - 1;
                    range2.newListEnd = snakeDiffPartial.f72y;
                } else {
                    range2.oldListEnd = snakeDiffPartial.f71x;
                    range2.newListEnd = snakeDiffPartial.f72y - 1;
                }
                arrayList2.add(range2);
                if (snakeDiffPartial.reverse) {
                    if (snakeDiffPartial.removal) {
                        range.oldListStart = snakeDiffPartial.f71x + snakeDiffPartial.size + 1;
                        range.newListStart = snakeDiffPartial.f72y + snakeDiffPartial.size;
                    } else {
                        range.oldListStart = snakeDiffPartial.f71x + snakeDiffPartial.size;
                        range.newListStart = snakeDiffPartial.f72y + snakeDiffPartial.size + 1;
                    }
                } else {
                    range.oldListStart = snakeDiffPartial.f71x + snakeDiffPartial.size;
                    range.newListStart = snakeDiffPartial.f72y + snakeDiffPartial.size;
    ...
    */
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake != null && snake.f71x == 0 && snake.f72y == 0) {
                return;
            }
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f71x + snake.size;
                int i4 = snake.f72y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f71x + i5;
                    int i7 = snake.f72y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i7 = snake.f71x + snake.size;
                int i8 = snake.f72y + snake.size;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f71x + i3;
                int i5 = snake.f72y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f71x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f71x + i6, 1, this.mCallback.getChangePayload(snake.f71x + i6, snake.f72y + i6));
                    }
                }
                i = snake.f71x;
                i2 = snake.f72y;
