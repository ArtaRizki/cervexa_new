...
public final class OperatorSwitch<T> implements Observable.Operator<T, Observable<? extends T>> {
    ...
    private static final class SwitchSubscriber<T> extends Subscriber<Observable<? extends T>> {
        ...
        final NotificationLite<T> f3506nl = NotificationLite.instance();
        ...
        void emit(T t, InnerSubscriber<T> innerSubscriber) {
            synchronized (this) {
                if (this.index.get() != ((InnerSubscriber) innerSubscriber).f3505id) {
                    return;
                }
                this.queue.offer(innerSubscriber, this.f3506nl.next(t));
                drain();
            }
        }
            ...
            synchronized (this) {
                ...
                while (true) {
                    ...
                    while (j3 != j2) {
                        ...
                        if (checkTerminated(z, z3, th2, spscLinkedArrayQueue, subscriber, zIsEmpty)) {
                            return;
                        }
                        if (zIsEmpty) {
                            break;
                        ...
                        T value = this.f3506nl.getValue(spscLinkedArrayQueue.poll());
                        if (atomicLong.get() == innerSubscriber.f3505id) {
    ...
    static final class InnerSubscriber<T> extends Subscriber<T> {
        ...
        public void setProducer(Producer producer) {
            this.parent.innerProducer(producer, this.f3505id);
        }
        ...
        @Override // p035rx.Observer
        public void onError(Throwable th) {
            this.parent.error(th, this.f3505id);
        }

        @Override // p035rx.Observer
        public void onCompleted() {
            this.parent.complete(this.f3505id);
        }
    }
}
