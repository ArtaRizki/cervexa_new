...
public final class OperatorWindowWithObservable<T, U> implements Observable.Operator<Observable<T>, T> {
    ...
    static final NotificationLite<Object> f3509nl = NotificationLite.instance();
    ...
    static final class SourceSubscriber<T> extends Subscriber<T> {
        ...
        void drain(List<Object> list) {
            ...
            for (Object obj : list) {
                if (obj == OperatorWindowWithObservable.NEXT_SUBJECT) {
                    replaceSubject();
                } else if (OperatorWindowWithObservable.f3509nl.isError(obj)) {
                    error(OperatorWindowWithObservable.f3509nl.getError(obj));
                    return;
                } else {
                    if (OperatorWindowWithObservable.f3509nl.isCompleted(obj)) {
                        complete();
                        return;
                    }
                    emitValue(obj);
        ...
        public void onError(Throwable th) {
            synchronized (this.guard) {
                if (this.emitting) {
                    this.queue = Collections.singletonList(OperatorWindowWithObservable.f3509nl.error(th));
                    return;
                }
                this.queue = null;
                this.emitting = true;
        ...
        public void onCompleted() {
            synchronized (this.guard) {
                if (this.emitting) {
                    ...
                    this.queue.add(OperatorWindowWithObservable.f3509nl.completed());
                    return;
                }
                List<Object> list = this.queue;
                this.queue = null;
