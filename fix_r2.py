...
for root, dirs, files in os.walk(java_dir):
    for file in files:
        if file.endswith('.java') and not file.endswith('R.java'):
            ...
            # Replace all C...R. with fully qualified R
            if c_r_usage_pattern.search(new_content):
                new_content = c_r_usage_pattern.sub('com.weioa.KmedHealthIndonesia.R.', new_content)
                # Remove the C...R imports entirely
                new_content = c_r_import_pattern.sub('', new_content)
